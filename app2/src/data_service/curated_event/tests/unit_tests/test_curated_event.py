...
class TestCuratedEvent:
    ...
    def setup_dynamodb(
        ...
        for table_attributes in tables_attributes:
            ...
            assert dynamodb_tuple.client.describe_table(TableName=table_attributes["TableName"])["Table"]["TableName"] == table_attributes["TableName"]

    @pytest.fixture
    def kinesis_records(self, mock_dynamodb, mock_table, mock_kinesis, mock_stream):  # pylint: disable=unused-argument
        stream_arn = mock_dynamodb.client.describe_table(TableName=f"{self.prefix.lower()}-{self.event_table_name.lower()}")["Table"]["LatestStreamArn"]
        logger.debug(f"{stream_arn=}")
        logger.info(f"fixture:kinesis_records inserting records into table_name={self.prefix.lower()}-{self.event_table_name.lower()}")
    ...
    def test_valid_base64_decode_success(self, kinesis_records):
        for record in kinesis_records["Records"]:
            logger.info(f"{record=}")
            data = curated_event.deserialize(record["kinesis"]["data"], ["INSERT"])
            logger.info(f"{data=}")
            logger.info(f"{type(data)=}")
            assert isinstance(data, LightningLaneEvent)

    def test_no_dynamodb_action(self, kinesis_records):
        for record in kinesis_records["Records"]:
            with pytest.raises(builtins.ValueError):
                logger.info(f"{record=}")
                curated_event.deserialize(record["kinesis"]["data"], ["UPDATE"])

    def test_invalid_base64_decode_error(self):
        with pytest.raises(builtins.ValueError):
            record = self.bad_kinesis_record["kinesis"]["data"]
            logger.info(f"{record=}")
            curated_event.deserialize(record, ["INSERT"])
