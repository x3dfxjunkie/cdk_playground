...
class QueryObject:
    """Stores parameters to be built into a CTAM query."""
...
def build_query(query_object: QueryObject) -> str:
    """Build CTAM specific query given a QueryObject."""
    if not query_object.selected_field_names:
        raise ValueError("No fields were selected for CTAM Query")
    ...
    # RetrieveData allows data filtering based on entity primary ID fields
        ...
        conditions = add_or_conditions(
            "=", conditions, query_object.id_field_name, query_object.fields, query_object.data_type
        )
        if conditions:
...
def add_or_conditions(
    operator: str, conditions: List[str], variable: str, values: List[str], value_type: str
) -> List[str]:
...
def add_condition(operator: str, conditions: List[str], field_name: str, value: str, value_type: str) -> str:
...
def translate_filter_expression(filter_expression: str, entity_definition: EntityDefinition) -> str:
    """Converts filter expression to CTAM specific expression using FilterExpressionVisitor."""
    if filter_expression:
        parse_tree = parse(filter_expression)
        print(entity_definition, parse_tree)
        # filter_expression_title = CTAMQueryFilterExpressionTitle(entity_definition)
        # filter_expression_title.visit(parse_tree)
    return ""
