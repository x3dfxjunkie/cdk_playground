...
@pytest.fixture
def test_props():
    with open(f"{os.path.dirname(__file__)}/config/kinesis_pipeline.yaml", encoding="utf-8") as fp:
...
def test_kinesis_ingest_created(test_props, test_stack):
    ...
    with patch(
        ...
        Kinesis(test_stack, "test-dms-ingest", test_props)
