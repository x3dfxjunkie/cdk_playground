{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__unappl_ppty_cleansed'),
            join_columns=['data_unappl_ppty_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__unappl_ppty_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_unappl_ppty_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_unappl_ppty_id']
    ) }}
),
...
final as (
    select
        data_unappl_ppty_id,
        data_usr_data_entry_nm,
        data_folio_txn_usr_data_entry_vl,
