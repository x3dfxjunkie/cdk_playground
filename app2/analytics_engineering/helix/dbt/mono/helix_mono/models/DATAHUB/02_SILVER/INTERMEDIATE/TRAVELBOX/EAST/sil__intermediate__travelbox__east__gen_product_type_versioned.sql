{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__east__gen_product_type_cleansed'),
            join_columns=['data_type_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__east__gen_product_type_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_type_id']
    ) }}
),
...
final as (
    select
        ...
        data_code,
        data_description,
        data_departure_times_compulsory,
        data_modified_on,
        metadata_checksum,
        metadata_timestamp,
