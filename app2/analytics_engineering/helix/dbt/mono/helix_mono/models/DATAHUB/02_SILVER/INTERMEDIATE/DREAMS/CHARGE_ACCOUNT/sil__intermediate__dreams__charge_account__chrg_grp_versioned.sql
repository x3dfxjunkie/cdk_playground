{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__charge_account__chrg_grp_cleansed'),
            join_columns=['data_chrg_grp_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__charge_account__chrg_grp_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_chrg_grp_id']
    ) }}
),
...
final as (
    select
        data_auth_pin_encrpt_hash_vl,
        data_chrg_grp_id,
        data_chrg_grp_sts_nm,
        data_chrg_grp_typ_nm,
        data_chrg_grp_ds,
        data_chrg_grp_strt_dts,
        data_chrg_grp_end_dts,
        data_txn_fac_id,
        data_src_acct_ctr_id,
        data_grp_dlgt_sml_bal_wrtoff_in,
        data_create_usr_id_cd,
        data_create_dts,
        data_updt_usr_id_cd,
        data_updt_dts,
        data_jdo_seq_nb,
        data_chrg_grp_actv_in,
        metadata_checksum,
        metadata_timestamp,
