{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__guest_link__xband_cleansed'),
            join_columns=['data_exprnc_media_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__guest_link__xband_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_media_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_media_id']
    ) }}
),
...
final as (
    select
        ...
        data_hf_secur_xband_id,
        data_jdo_seq_nb,
        data_mfr_uid,
        data_updt_dts,
        data_updt_usr_id_cd,
        data_xband_sec_sts_nm,
        data_xband_sts_nm,
        metadata_checksum,
        metadata_timestamp,
