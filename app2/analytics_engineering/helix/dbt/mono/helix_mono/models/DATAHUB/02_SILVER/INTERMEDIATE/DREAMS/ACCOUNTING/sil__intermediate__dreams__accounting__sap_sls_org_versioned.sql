{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__sap_sls_org_cleansed'),
            join_columns=['data_sap_sls_org_cd'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__sap_sls_org_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_sap_sls_org_cd
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_sap_sls_org_cd']
    ) }}
),
