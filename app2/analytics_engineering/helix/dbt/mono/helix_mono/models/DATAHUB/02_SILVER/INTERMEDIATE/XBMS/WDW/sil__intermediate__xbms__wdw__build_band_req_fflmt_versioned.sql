{{
    config(
        ...
        unique_key = ['data_build_band_ord_fflmt_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__build_band_req_fflmt_cleansed'),
            join_columns=['data_build_band_ord_fflmt_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__build_band_req_fflmt_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_build_band_ord_fflmt_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_build_band_ord_fflmt_id']
    ) }}
),
