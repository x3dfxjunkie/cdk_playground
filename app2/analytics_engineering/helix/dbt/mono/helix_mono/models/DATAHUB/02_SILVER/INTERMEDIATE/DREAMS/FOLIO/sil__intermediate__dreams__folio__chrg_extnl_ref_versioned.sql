{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__chrg_extnl_ref_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__chrg_extnl_ref_cleansed') }}
    {% if is_incremental() %}
