{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__inbound_rest_req_log_cleansed'),
            join_columns=['data_correlation_id', 'data_req_timestamp'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__inbound_rest_req_log_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_correlation_id, data_req_timestamp
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_correlation_id', 'data_req_timestamp']
    ) }}
),
