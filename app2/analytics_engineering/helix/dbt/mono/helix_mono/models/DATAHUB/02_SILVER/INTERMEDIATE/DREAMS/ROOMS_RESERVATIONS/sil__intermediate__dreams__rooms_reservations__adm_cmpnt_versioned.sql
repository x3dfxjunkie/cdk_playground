{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_reservations__adm_cmpnt_cleansed'),
            join_columns=['data_adm_tc_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_reservations__adm_cmpnt_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_adm_tc_id']
    ) }}
),
...
final as (
    select
        ...
        data_updt_dts,
        data_updt_usr_id_cd,
        data_validity_end_dt,
        data_validity_strt_dt,
        data_prge_dts,
        metadata_checksum,
        metadata_timestamp,
