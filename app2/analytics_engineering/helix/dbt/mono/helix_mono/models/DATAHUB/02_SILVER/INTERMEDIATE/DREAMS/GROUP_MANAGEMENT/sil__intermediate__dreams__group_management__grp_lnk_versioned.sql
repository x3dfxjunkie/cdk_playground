{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__group_management__grp_lnk_cleansed'),
            join_columns=['data_prmy_grp_cd', 'data_sec_grp_cd'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__group_management__grp_lnk_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_prmy_grp_cd, data_sec_grp_cd
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_prmy_grp_cd', 'data_sec_grp_cd']
    ) }}
),
