{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_reservations__whsl_pkg_excpt_cleansed'),
            join_columns=['data_pkg_excpt_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_reservations__whsl_pkg_excpt_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_pkg_excpt_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pkg_excpt_id']
    ) }}
),
...
final as (
    select
        ...
        data_jdo_seq_nb,
        data_pkg_excpt_id,
        data_pkg_excpt_typ_nm,
        data_pkg_excpt_sts_nm,
        data_pkg_excpt_st_nm,
        data_tp_id,
        data_tps_id,
        data_tc_grp_nb,
        data_grp_nm,
