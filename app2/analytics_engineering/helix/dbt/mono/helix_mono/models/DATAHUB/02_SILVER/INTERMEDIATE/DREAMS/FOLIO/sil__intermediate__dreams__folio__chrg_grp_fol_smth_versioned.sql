{{
    config(
        ...
        unique_key = ['data_chrg_grp_folio_id', 'data_settl_meth_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__chrg_grp_fol_smth_cleansed'),
            join_columns=['data_chrg_grp_folio_id', 'data_settl_meth_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__chrg_grp_fol_smth_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_chrg_grp_folio_id, data_settl_meth_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_chrg_grp_folio_id', 'data_settl_meth_id']
    ) }}
),
