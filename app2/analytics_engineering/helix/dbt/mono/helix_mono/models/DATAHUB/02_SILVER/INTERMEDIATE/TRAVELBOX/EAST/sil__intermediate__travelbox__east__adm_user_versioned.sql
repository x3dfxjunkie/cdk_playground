{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__east__adm_user_cleansed'),
            join_columns=['data_user_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__east__adm_user_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_user_id']
    ) }}
),
...
final as (
    select
        ...
        data_notes,
        data_created_by_agent,
        data_default_answer,
        data_default_currency,
        data_default_div,
        data_default_com,
        data_agent_initials,
        data_agent_sign,
        ...
        data_question_id,
        data_answer,
        data_pwd_change_date,
        data_default_password,
        metadata_checksum,
        metadata_timestamp,
