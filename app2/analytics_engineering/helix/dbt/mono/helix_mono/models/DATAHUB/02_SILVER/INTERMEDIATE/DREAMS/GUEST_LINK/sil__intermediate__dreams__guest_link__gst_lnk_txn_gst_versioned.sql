{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__guest_link__gst_lnk_txn_gst_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__guest_link__gst_lnk_txn_gst_cleansed') }}
    {% if is_incremental() %}
