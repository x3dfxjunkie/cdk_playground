{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__age_def_t_cleansed'),
            join_columns=['data_age_def_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__age_def_t_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_age_def_id']
    ) }}
),
...
final as (
    select
        data_age_def_id,
        data_age_typ_nm,
        data_entrprs_age_typ_id,
        data_age_def_min_age_nb,
        data_age_def_max_age_nb,
        data_entrprs_age_typ_nm,
        data_create_dts,
        data_updt_dts,
        data_jdo_seq_nb,
        metadata_checksum,
