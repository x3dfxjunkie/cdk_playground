{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__east__act_client_refund_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__east__act_client_refund_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        ...
        data_refund_type,
        data_authorised,
        data_transfer_receipt_index,
        data_refund_batch_no,
        data_auto_refund,
        data_comm_payment_as_refund,
        data_comm_payment_printed,
        data_temp_user,
        data_authorised_user,
        data_cheque_refund_index,
        metadata_checksum,
        metadata_timestamp,
