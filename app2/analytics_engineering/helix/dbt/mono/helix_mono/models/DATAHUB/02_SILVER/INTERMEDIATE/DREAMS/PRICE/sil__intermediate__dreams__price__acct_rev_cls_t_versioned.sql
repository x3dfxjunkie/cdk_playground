{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__acct_rev_cls_t_cleansed'),
            join_columns=['data_acct_rev_cls_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__acct_rev_cls_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_acct_rev_cls_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_acct_rev_cls_id']
    ) }}
),
...
final as (
    select
        ...
        data_acct_rev_cls_lvl_nb,
        data_acct_rev_cls_nm,
        data_create_dts,
        data_jdo_seq_nb,
        data_prnt_acct_rev_cls_id,
        data_updt_dts,
        metadata_checksum,
        metadata_timestamp,
