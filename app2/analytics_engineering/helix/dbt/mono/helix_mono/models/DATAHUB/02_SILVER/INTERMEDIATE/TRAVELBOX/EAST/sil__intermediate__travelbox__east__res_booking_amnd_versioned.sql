{{
    config(
        ...
        unique_key = ['data_amnd_no', 'data_booking_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__east__res_booking_amnd_cleansed'),
            join_columns=['data_amnd_no', 'data_booking_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__east__res_booking_amnd_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_amnd_no, data_booking_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_amnd_no', 'data_booking_id']
    ) }}
),
