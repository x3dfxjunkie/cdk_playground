{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__west__act_client_receipt_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__west__act_client_receipt_cleansed') }}
    {% if is_incremental() %}
