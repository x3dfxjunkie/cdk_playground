{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__west__cli_comm_group_cleansed'),
            join_columns=['data_comm_group_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__west__cli_comm_group_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_comm_group_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_comm_group_id']
    ) }}
),
...
final as (
    select
        ...
        data_sales_priority,
        data_currency,
        data_applicable_date_type,
        data_applicable_product,
        data_applicable_limit_booking,
        data_external_ref,
        data_com_grp_priority,
        metadata_checksum,
        metadata_timestamp,
