{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__east__res_booking_fee_cleansed'),
            join_columns=['data_booking_id', 'data_fee_code', 'data_fee_code_index'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__east__res_booking_fee_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_booking_id, data_fee_code, data_fee_code_index
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_booking_id', 'data_fee_code', 'data_fee_code_index']
    ) }}
),
