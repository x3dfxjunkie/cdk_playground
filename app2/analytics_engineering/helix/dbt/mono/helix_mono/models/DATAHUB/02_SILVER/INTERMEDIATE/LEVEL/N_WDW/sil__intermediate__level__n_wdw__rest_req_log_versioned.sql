{{
    config(
        ...
        unique_key = ['data_correlation_id', 'data_req_timestamp_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__rest_req_log_cleansed'),
            join_columns=['data_correlation_id', 'data_req_timestamp_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__rest_req_log_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_correlation_id, data_req_timestamp_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_correlation_id', 'data_req_timestamp_id']
    ) }}
),
