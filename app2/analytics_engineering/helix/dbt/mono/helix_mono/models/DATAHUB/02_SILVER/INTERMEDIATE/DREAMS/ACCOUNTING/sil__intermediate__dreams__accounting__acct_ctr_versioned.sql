{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__acct_ctr_cleansed'),
            join_columns=['data_acct_ctr_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__acct_ctr_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_acct_ctr_id']
    ) }}
),
...
final as (
    select
        data_acct_ctr_cd,
        data_acct_ctr_dspl_seq_nb,
        data_acct_ctr_id,
        data_acct_ctr_nm,
        data_chrg_acct_ctr_in,
        data_cmps_id,
        data_create_dts,
        data_create_usr_id_cd,
        data_db_wroff_max_am,
        data_db_wroff_max_crncy_cd,
        data_fac_id,
        data_jdo_seq_nb,
        data_req_nm,
        data_updt_dts,
