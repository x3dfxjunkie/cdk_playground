{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__mmdp_ticket_price_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__mmdp_ticket_price_cleansed') }}
    {% if is_incremental() %}
