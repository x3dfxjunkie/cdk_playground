{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__acct_ctr_rfnd_pmth_cleansed'),
            join_columns=['data_acct_ctr_rfnd_pmt_meth_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__acct_ctr_rfnd_pmth_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_acct_ctr_rfnd_pmt_meth_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_acct_ctr_rfnd_pmt_meth_id']
    ) }}
),
...
final as (
    select
        ...
        data_rfnd_pmt_meth_id,
        data_dflt_rfnd_pmt_meth_in,
        data_rfnd_pmt_meth_seq_nb,
