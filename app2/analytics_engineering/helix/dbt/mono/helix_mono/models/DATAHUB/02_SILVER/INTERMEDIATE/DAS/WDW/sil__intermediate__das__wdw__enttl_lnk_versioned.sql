{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__das__wdw__enttl_lnk_cleansed'),
            join_columns=['data_enttl_id', 'data_lnk_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__das__wdw__enttl_lnk_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_enttl_id, data_lnk_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_enttl_id', 'data_lnk_id']
    ) }}
),
...
final as (
    select
        data_enttl_id,
        data_lnk_id,
        data_enttl_lnk_strt_dts,
        data_enttl_lnk_end_dts,
        data_gwd_rl,
        data_under3,
        data_hs_opt_out,
        data_create_usr_id,
