{{
    config(
        ...
        unique_key = ['data_cmpnt_prod_id', 'data_price_grid_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__prod_price_grid_t_cleansed'),
            join_columns=['data_cmpnt_prod_id', 'data_price_grid_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__prod_price_grid_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_cmpnt_prod_id, data_price_grid_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_cmpnt_prod_id', 'data_price_grid_id']
    ) }}
),
