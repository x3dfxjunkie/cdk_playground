{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__west__pkg_itinerary_cleansed'),
            join_columns=['data_itinerary_no', 'data_package_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__west__pkg_itinerary_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_itinerary_no, data_package_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_itinerary_no', 'data_package_id']
    ) }}
),
...
final as (
    select
        ...
        data_hotel_element_no,
        data_max_bkg_to_dep_days,
        data_dest_city_code,
        ...
        data_calc_pax_base,
        data_dep_date_exist,
        data_push_sales_exist,
        data_special_offer_exist,
        data_blackout_exist,
        data_ebr_discount_exist,
        data_visa_exist,
        data_cms_code,
        data_iti_dest_city,
        data_country,
