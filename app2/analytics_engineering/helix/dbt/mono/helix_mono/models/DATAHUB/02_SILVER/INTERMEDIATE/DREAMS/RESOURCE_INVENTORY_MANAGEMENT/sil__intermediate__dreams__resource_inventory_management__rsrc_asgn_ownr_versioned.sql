{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__resource_inventory_management__rsrc_asgn_ownr_cleansed'),
            join_columns=['data_asgn_ownr_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__resource_inventory_management__rsrc_asgn_ownr_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_asgn_ownr_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_asgn_ownr_id']
    ) }}
),
...
final as (
    select
        ...
        data_jdo_seq_nb,
        data_ownr_end_dts,
        data_ownr_rsrt_trfr_in,
        data_ownr_strt_dts,
        data_ownr_sts_nm,
        data_rsrc_invtry_typ_id,
        data_spcl_need_req_in,
        data_updt_dts,
        data_updt_usr_id_cd,
        data_vip_in,
        data_whsl_in,
        data_prnt_asgn_ownr_id,
        data_eff_rt_ctgy_nm,
        data_auto_asgn_rsrc_id,
        data_blk_cd,
        data_grp_id_vl,
        data_dvc_own_lnk_id,
        data_prge_dts,
