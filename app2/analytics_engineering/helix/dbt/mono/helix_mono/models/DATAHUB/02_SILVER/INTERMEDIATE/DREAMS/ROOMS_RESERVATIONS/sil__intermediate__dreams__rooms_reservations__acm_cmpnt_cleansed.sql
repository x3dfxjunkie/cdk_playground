{{
    config(
        ...
        unique_key= ['data_acm_tc_id','metadata_checksum'],
...
with source as (
    ...
    from {{ source('brz_dreams_rooms_reservations', 'acm_cmpnt') }}
...
renamed as (
    select
        ...
        TRIM(src.data_bus_nb) as data_bus_nb,
        ...
        TRIM(src.data_lt_chkot_in) as data_lt_chkot_in,
    ...
    on src.data_acm_tc_id = min_source_update_datetime.data_acm_tc_id
),
...
final as (
    {{ dbt_utils.deduplicate(
        relation = 'renamed',
        partition_by = 'data_acm_tc_id, metadata_checksum',
