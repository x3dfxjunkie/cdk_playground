{{
    config(
        ...
        unique_key = ['data_grp_bus_src_nm','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__group_management__grp_bus_src_cleansed'),
            join_columns=['data_grp_bus_src_nm'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__group_management__grp_bus_src_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_grp_bus_src_nm
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_grp_bus_src_nm']
    ) }}
),
