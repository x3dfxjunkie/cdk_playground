{{
    config(
        ...
        unique_key = ['data_comnctn_chan_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_reservations__comnctn_chan_cleansed'),
            join_columns=['data_comnctn_chan_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_reservations__comnctn_chan_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_comnctn_chan_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_comnctn_chan_id']
    ) }}
),
