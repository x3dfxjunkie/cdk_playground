{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__trvl_agcy_cleansed'),
            join_columns=['data_trvl_agcy_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__trvl_agcy_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_trvl_agcy_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_trvl_agcy_id']
    ) }}
),
...
final as (
    select
        ...
        data_glbl_tkt_comm_in,
        data_iata_id,
        data_jdo_seq_nb,
        data_sap_vndr_id,
        data_trvl_agcy_actv_in,
        data_trvl_agcy_fm_create_dt,
        data_trvl_agcy_id,
        data_trvl_agcy_nm,
        data_trvl_agcy_ods_id,
        data_trvl_agcy_pay_comm_in,
        data_updt_dts,
        data_updt_usr_id_cd,
        metadata_checksum,
        metadata_timestamp,
