{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__settl_meth_cleansed'),
            join_columns=['data_settl_meth_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__settl_meth_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_settl_meth_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_settl_meth_id']
    ) }}
),
...
final as (
    select
        ...
        data_prge_dts,
        data_settl_meth_actv_in,
        data_settl_meth_create_dt,
        data_settl_meth_end_dt,
        data_settl_meth_id,
        data_settl_meth_inactv_dts,
        data_settl_meth_strt_dt,
        data_updt_dts,
        data_updt_usr_id_cd,
        metadata_checksum,
        metadata_timestamp,
