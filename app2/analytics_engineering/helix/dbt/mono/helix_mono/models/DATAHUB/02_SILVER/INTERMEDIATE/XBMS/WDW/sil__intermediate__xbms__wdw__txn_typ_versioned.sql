{{
    config(
        ...
        unique_key = ['data_txn_typ_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__txn_typ_cleansed'),
            join_columns=['data_txn_typ_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__txn_typ_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_txn_typ_id']
    ) }}
),
