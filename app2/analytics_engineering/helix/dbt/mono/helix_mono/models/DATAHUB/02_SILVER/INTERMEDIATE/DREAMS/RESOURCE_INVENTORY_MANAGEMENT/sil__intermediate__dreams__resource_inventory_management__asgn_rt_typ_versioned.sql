{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__resource_inventory_management__asgn_rt_typ_cleansed'),
            join_columns=['data_rsrt_asgn_scr_id', 'data_rt_typ_nm'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__resource_inventory_management__asgn_rt_typ_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_rsrt_asgn_scr_id, data_rt_typ_nm
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_rsrt_asgn_scr_id', 'data_rt_typ_nm']
    ) }}
),
