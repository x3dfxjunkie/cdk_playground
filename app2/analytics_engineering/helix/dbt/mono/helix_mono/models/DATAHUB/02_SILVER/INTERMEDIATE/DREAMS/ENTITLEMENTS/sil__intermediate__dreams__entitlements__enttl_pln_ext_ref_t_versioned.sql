{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__entitlements__enttl_pln_ext_ref_t_cleansed'),
            join_columns=['data_enttl_plan_ext_ref_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__entitlements__enttl_pln_ext_ref_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_enttl_plan_ext_ref_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_enttl_plan_ext_ref_id']
    ) }}
),
...
final as (
    select
        ...
        data_enttl_extnl_src_nm,
        data_enttl_extnl_src_ref_vl,
        data_enttl_ext_ref_typ_nm,
