{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__app_config_dtls_cleansed'),
            join_columns=['data_app_config_dtls_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__app_config_dtls_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_app_config_dtls_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_app_config_dtls_id']
    ) }}
),
