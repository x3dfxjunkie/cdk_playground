{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__tkt_price_grid_t_cleansed'),
            join_columns=['data_tkt_price_grid_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__tkt_price_grid_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_tkt_price_grid_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_tkt_price_grid_id']
    ) }}
),
...
final as (
    select
        ...
        data_price_grid_tkt_feat_in,
        data_price_grid_tkt_cnvntn_in,
        data_price_grid_tkt_intfc_cd,
        data_price_grid_tkt_fsell_in,
        data_tkt_sl_eff_dt,
        data_tkt_sl_exp_dt,
        data_prge_dts,
        metadata_checksum,
