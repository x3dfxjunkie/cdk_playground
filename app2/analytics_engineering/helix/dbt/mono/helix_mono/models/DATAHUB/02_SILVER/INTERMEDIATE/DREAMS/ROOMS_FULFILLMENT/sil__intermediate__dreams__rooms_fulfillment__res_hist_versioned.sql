{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_fulfillment__res_hist_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_fulfillment__res_hist_cleansed') }}
    {% if is_incremental() %}
