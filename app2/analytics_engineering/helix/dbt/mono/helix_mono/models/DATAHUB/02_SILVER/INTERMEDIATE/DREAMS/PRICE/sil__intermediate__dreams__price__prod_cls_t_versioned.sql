{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__prod_cls_t_cleansed'),
            join_columns=['data_prod_cls_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__prod_cls_t_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_prod_cls_id']
    ) }}
),
...
final as (
    select
        ...
        data_jdo_cls_nb,
        data_jdo_seq_nb,
        data_price_calc_typ_nm,
        data_prnt_prod_cls_id,
        data_prod_cls_id,
        data_prod_cls_lvl_nb,
        data_prod_cls_nm,
        data_updt_dts,
        data_updt_usr_id_cd,
        metadata_checksum,
