{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__pkg_cmpnt_reord_config_cleansed'),
            join_columns=['data_pkg_cmpnt_reord_config_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__pkg_cmpnt_reord_config_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_pkg_cmpnt_reord_config_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pkg_cmpnt_reord_config_id']
    ) }}
),
