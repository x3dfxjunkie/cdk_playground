{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_reservations__tp_gthr_cleansed'),
            join_columns=['data_gthr_cd', 'data_tp_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_reservations__tp_gthr_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_gthr_cd, data_tp_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_gthr_cd', 'data_tp_id']
    ) }}
),
