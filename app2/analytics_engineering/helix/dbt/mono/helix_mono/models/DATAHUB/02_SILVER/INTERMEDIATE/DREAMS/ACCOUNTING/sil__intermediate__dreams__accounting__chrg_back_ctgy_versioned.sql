{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__chrg_back_ctgy_cleansed'),
            join_columns=['data_chrg_back_ctgy_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__chrg_back_ctgy_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_chrg_back_ctgy_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_chrg_back_ctgy_id']
    ) }}
),
...
final as (
    select
        ...
        data_chrg_back_ctgy_nm,
        data_chrg_back_ctgy_strt_dt,
        data_chrg_back_ctgy_end_dt,
