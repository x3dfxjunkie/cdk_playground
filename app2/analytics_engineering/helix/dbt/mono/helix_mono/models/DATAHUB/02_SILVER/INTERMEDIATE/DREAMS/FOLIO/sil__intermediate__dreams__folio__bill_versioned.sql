{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__bill_cleansed'),
            join_columns=['data_bill_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__bill_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_bill_id']
    ) }}
),
...
final as (
    select
        ...
        data_bill_cd_nm,
        data_bill_cd_ds,
        data_bill_cd_inactv_dts,
        data_bill_cd_strt_dt,
        data_bill_cd_end_dt,
