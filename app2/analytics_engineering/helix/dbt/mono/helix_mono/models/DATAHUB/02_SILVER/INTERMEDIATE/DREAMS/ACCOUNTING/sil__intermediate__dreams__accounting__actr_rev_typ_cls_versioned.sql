{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__actr_rev_typ_cls_cleansed'),
            join_columns=['data_acct_ctr_rev_typ_cls_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__actr_rev_typ_cls_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_acct_ctr_rev_typ_cls_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_acct_ctr_rev_typ_cls_id']
    ) }}
),
...
final as (
    select
        ...
        data_rev_cls_id,
        data_tax_grp_id,
        data_dflt_actr_rev_cls_in,
        data_actr_rev_cls_actv_in,
        data_allow_man_pst_in,
