{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_reservations__tc_grp_cleansed'),
            join_columns=['data_tc_grp_nb'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_reservations__tc_grp_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_tc_grp_nb']
    ) }}
),
...
final as (
    select
        ...
        data_sls_chan_id,
        data_tc_grp_typ_nm,
        data_adv_internt_chkin_in,
        data_add_on_tc_grp_nb,
        data_prge_dts,
        metadata_checksum,
        metadata_timestamp,
