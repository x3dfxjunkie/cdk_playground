{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__pst_grp_cleansed'),
            join_columns=['data_pst_grp_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__pst_grp_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pst_grp_id']
    ) }}
),
...
final as (
    select
        data_pst_grp_id,
        data_pst_grp_nm,
        data_pst_grp_actv_in,
        data_pst_grp_tax_dtrmnt_in,
        data_pst_grp_dspl_seq_nb,
