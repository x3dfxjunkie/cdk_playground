{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__folio_item_cleansed'),
            join_columns=['data_folio_item_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__folio_item_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_folio_item_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_folio_item_id']
    ) }}
),
...
final as (
    select
        data_folio_item_id,
        data_folio_id,
        data_folio_item_typ_nm,
        data_folio_item_am,
        data_folio_item_crncy_cd,
