{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_fulfillment__exprnc_enhnc_prod_cleansed'),
            join_columns=['data_exprnc_enhnc_nm', 'data_prod_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_fulfillment__exprnc_enhnc_prod_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_enhnc_nm, data_prod_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_enhnc_nm', 'data_prod_id']
    ) }}
),
...
final as (
    select
        ...
        data_exprnc_enhnc_prod_end_dt,
        data_exprnc_enhnc_prod_strt_dt,
        data_jdo_seq_nb,
        data_prge_dts,
        data_prod_id,
        data_updt_dts,
