{{
    config(
        ...
        unique_key = ['data_sap_mtrl_cd','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__sap_mtrl_cleansed'),
            join_columns=['data_sap_mtrl_cd'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__sap_mtrl_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_sap_mtrl_cd']
    ) }}
),
