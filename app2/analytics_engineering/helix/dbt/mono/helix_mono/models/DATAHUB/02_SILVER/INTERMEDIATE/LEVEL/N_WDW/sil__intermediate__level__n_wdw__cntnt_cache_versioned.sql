{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__cntnt_cache_cleansed'),
            join_columns=['data_cntnt_cache_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__cntnt_cache_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_cntnt_cache_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_cntnt_cache_id']
    ) }}
),
...
final as (
    select
        data_cntnt_cache_id,
        data_entrprs_cntnt_typ_nm,
        data_entrprs_cntnt_id,
        data_cntnt_attr_nm,
        data_cntnt_attr_tx,
        data_create_usr_id,
        data_create_dts,
