{{
    config(
        ...
        unique_key = ['data_marital_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__east__cli_marital_status_cleansed'),
            join_columns=['data_marital_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__east__cli_marital_status_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_marital_id']
    ) }}
),
