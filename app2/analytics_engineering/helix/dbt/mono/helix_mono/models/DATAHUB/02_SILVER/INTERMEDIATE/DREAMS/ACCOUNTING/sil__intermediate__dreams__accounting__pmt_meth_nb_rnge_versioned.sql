{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__pmt_meth_nb_rnge_cleansed'),
            join_columns=['data_pmt_meth_nb_rnge_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__pmt_meth_nb_rnge_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_pmt_meth_nb_rnge_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pmt_meth_nb_rnge_id']
    ) }}
),
...
final as (
    select
        ...
        data_pmt_meth_id,
        data_pmt_meth_rnge_st_nb,
        data_pmt_meth_rnge_end_nb,
        data_pmt_meth_nb_rnge_strt_dt,
        data_pmt_meth_nb_rnge_end_dt,
