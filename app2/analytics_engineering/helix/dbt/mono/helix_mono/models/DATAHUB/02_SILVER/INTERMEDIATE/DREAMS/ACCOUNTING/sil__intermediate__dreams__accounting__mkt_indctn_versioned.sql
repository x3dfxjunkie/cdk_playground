{{
    config(
        ...
        unique_key = ['data_mkt_indctn_cd','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__mkt_indctn_cleansed'),
            join_columns=['data_mkt_indctn_cd'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__mkt_indctn_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_mkt_indctn_cd
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_mkt_indctn_cd']
    ) }}
),
