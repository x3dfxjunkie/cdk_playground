{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__shipmt_rcpt_item_log_cleansed'),
            join_columns=['data_shipmt_rcpt_item_log_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__shipmt_rcpt_item_log_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_shipmt_rcpt_item_log_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_shipmt_rcpt_item_log_id']
    ) }}
),
...
final as (
    select
        ...
        data_shipmt_rcpt_item_seq_nb,
        data_exprnc_band_sku_cd,
        data_po_nb,
        data_shipmt_nb,
        data_rcpnt_vndr_id,
        data_fflmt_vndr_job_nb,
        data_shipmt_rcpt_item_cn,
        data_shipmt_rcpt_item_dlvr_dt,
        data_shipmt_rcpt_gnrt_dt,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
