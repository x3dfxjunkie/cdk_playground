{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_req_hld_rsn_cleansed'),
            join_columns=['data_exprnc_band_req_hld_rsn_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_req_hld_rsn_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_band_req_hld_rsn_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_band_req_hld_rsn_id']
    ) }}
),
...
final as (
    select
        ...
        data_exprnc_band_req_hld_rsn_cd,
        data_exprnc_band_req_hld_rsn_ds,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_logical_del_in,
        data_req_hld_rsn_cst_dspl_in,
