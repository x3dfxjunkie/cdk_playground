{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__east__cancellation_scheme_cleansed'),
            join_columns=['data_cancel_scheme_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__east__cancellation_scheme_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_cancel_scheme_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_cancel_scheme_id']
    ) }}
),
...
final as (
    select
        ...
        data_cancellation_ref,
        data_per_person_charge,
        data_max_days,
        data_charge_type,
        data_product_type,
        data_description,
