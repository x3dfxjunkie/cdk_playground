{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__xband_enttl_prod_assc_cleansed'),
            join_columns=['data_xband_enttl_prod_assc_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__xband_enttl_prod_assc_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_xband_enttl_prod_assc_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_xband_enttl_prod_assc_id']
    ) }}
),
