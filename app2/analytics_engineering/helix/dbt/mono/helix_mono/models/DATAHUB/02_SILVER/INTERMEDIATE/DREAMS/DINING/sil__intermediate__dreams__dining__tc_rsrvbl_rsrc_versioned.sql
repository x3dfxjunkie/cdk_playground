{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__dining__tc_rsrvbl_rsrc_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__dining__tc_rsrvbl_rsrc_cleansed') }}
    {% if is_incremental() %}
