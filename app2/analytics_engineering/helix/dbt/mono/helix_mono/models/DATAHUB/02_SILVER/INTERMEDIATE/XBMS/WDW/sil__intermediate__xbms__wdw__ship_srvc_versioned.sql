{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__ship_srvc_cleansed'),
            join_columns=['data_ship_srvc_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__ship_srvc_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_ship_srvc_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_ship_srvc_id']
    ) }}
),
...
final as (
    select
        data_ship_srvc_id,
        data_ship_srvc_cd,
        data_ship_srvc_fflmt_vndr_cd,
        data_ship_srvc_ds,
        data_ship_srvc_dlvr_di,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_logical_del_in,
        data_avail_to_all_users,
        metadata_checksum,
        metadata_timestamp,
