{{
    config(
        ...
        unique_key = ['data_entitlement_id', 'data_transaction_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__revenue_recognition_audit_entitlementid_cleansed'),
            join_columns=['data_entitlement_id', 'data_transaction_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__revenue_recognition_audit_entitlementid_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_entitlement_id, data_transaction_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_entitlement_id', 'data_transaction_id']
    ) }}
),
