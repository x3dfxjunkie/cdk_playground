{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__west__res_booking_cnx_cleansed'),
            join_columns=['data_booking_id', 'data_cnx_no'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__west__res_booking_cnx_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_booking_id, data_cnx_no
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_booking_id', 'data_cnx_no']
    ) }}
),
...
final as (
    select
        ...
        data_client_conf_doc_received,
        data_client_conf_doc_received_date,
        data_cnx_no,
        data_passenger_no,
