{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__natr_pitm_config_cleansed'),
            join_columns=['data_natr_pst_item_config_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__natr_pitm_config_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_natr_pst_item_config_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_natr_pst_item_config_id']
    ) }}
),
...
final as (
    select
        ...
        data_natr_pst_item_config_strt_dt,
        data_natr_pst_item_config_end_dt,
        data_pst_st_nm,
        data_folio_typ_nm,
