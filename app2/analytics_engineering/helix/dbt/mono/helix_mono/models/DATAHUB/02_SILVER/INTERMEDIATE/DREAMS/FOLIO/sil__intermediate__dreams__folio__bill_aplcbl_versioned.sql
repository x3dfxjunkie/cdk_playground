{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__bill_aplcbl_cleansed'),
            join_columns=['data_bill_aplcbl_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__bill_aplcbl_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_bill_aplcbl_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_bill_aplcbl_id']
    ) }}
),
...
final as (
    select
        data_bill_aplcbl_id,
        data_bill_id,
        data_bill_aplcbl_strt_dt,
        data_bill_aplcbl_end_dt,
        data_pkg_id,
