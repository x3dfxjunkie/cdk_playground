{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__alt_entry_cleansed'),
            join_columns=['data_anlss_ldgr_txn_entry_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__alt_entry_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_anlss_ldgr_txn_entry_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_anlss_ldgr_txn_entry_id']
    ) }}
),
...
final as (
    select
        ...
        data_pst_acct_id,
        data_pst_acct_ctr_id,
        data_chrg_acct_ctr_id,
        data_sap_gl_ref_id,
        data_chrg_acct_ctr_sap_id,
        data_natr_acct_in,
        data_pst_acct_am,
        data_pst_acct_crncy_cd,
