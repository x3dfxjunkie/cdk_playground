{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__group_management__grp_prfl_assc_cleansed'),
            join_columns=['data_grp_prfl_assc_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__group_management__grp_prfl_assc_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_grp_prfl_assc_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_grp_prfl_assc_id']
    ) }}
),
...
final as (
    select
        ...
        data_grp_cd,
        data_grp_prfl_assc_dglt_in,
        data_grp_prfl_assc_grp_in,
