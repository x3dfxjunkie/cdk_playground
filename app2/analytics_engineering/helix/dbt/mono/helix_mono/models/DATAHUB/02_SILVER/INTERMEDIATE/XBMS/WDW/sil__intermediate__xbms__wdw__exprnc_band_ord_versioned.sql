{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_ord_cleansed'),
            join_columns=['data_exprnc_band_ord_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_ord_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_band_ord_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_band_ord_id']
    ) }}
),
...
final as (
    select
        ...
        data_exprnc_band_ord_uniq_id,
        data_addr_id,
        data_exprnc_band_ord_expect_ship_dt,
        data_exprnc_band_ord_actl_dlvr_dt,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_logical_del_in,
        data_ship_srvc_id,
        data_exprnc_band_ord_expect_dlvr_dt,
        data_rsrt_shipmt_dsny_fac_id,
        data_curr_exprnc_band_ord_sts_id,
        data_redir_shipmt_to_rsrt,
        data_fflmt_loc_id,
