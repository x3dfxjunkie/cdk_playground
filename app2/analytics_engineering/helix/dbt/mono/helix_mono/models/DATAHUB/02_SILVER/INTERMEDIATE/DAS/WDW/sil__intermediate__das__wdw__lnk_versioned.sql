{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__das__wdw__lnk_cleansed'),
            join_columns=['data_lnk_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__das__wdw__lnk_cleansed') }}
    {% if is_incremental() %}
