{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__lvl_n_acct_rconcl_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__lvl_n_acct_rconcl_cleansed') }}
    {% if is_incremental() %}
