{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__folio_txn_cmt_cleansed'),
            join_columns=['data_folio_txn_cmt_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__folio_txn_cmt_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_folio_txn_cmt_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_folio_txn_cmt_id']
    ) }}
),
...
final as (
    select
        ...
        data_folio_txn_id,
        data_txn_rsn_cd,
        data_folio_txn_cmt_tx,
        data_folio_cmt_actv_in,
