{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__prod_chan_prod_t_cleansed'),
            join_columns=['data_prod_chan_id', 'data_prod_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__prod_chan_prod_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_prod_chan_id, data_prod_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_prod_chan_id', 'data_prod_id']
    ) }}
),
...
final as (
    select
        data_prod_chan_id,
        data_prod_chan_prod_end_dt,
        data_prod_chan_prod_strt_dt,
        data_prod_id,
        metadata_checksum,
        metadata_timestamp,
