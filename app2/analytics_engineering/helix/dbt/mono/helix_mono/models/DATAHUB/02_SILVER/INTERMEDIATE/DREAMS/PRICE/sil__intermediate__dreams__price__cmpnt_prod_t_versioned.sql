{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__cmpnt_prod_t_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__cmpnt_prod_t_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        data_cmpnt_prod_adlt_cpcty_nb,
        data_cmpnt_prod_cd,
        data_cmpnt_prod_cpcty_nb,
        data_cmpnt_prod_frnt_dsk_prt_in,
        data_cmpnt_prod_nm,
        data_dflt_price_sheet_id,
        data_enttl_frq_nm,
        data_enttl_meth_nm,
        data_gst_fcg_prod_nm,
        data_jdo_seq_nb,
        data_prge_dts,
        data_prod_id,
        data_tax_grp_id,
