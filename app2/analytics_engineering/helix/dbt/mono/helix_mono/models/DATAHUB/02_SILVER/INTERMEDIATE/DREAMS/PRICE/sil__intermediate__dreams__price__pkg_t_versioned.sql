{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__pkg_t_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__pkg_t_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        ...
        data_pkg_guar_in,
        data_pkg_id,
        data_pkg_prod_cls_nm,
        data_pkg_prod_comm_typ_nm,
        data_pkg_prod_grp_id,
        data_pkg_prod_grp_nm,
        data_pkg_prod_max_nb,
        data_pkg_prod_min_nb,
        data_pkg_prod_pln_typ_id,
        data_pkg_prod_src_sys_nm,
        data_plan_typ_nm,
        data_prge_dts,
        data_sub_pkg_in,
        metadata_checksum,
