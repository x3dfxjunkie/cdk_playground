{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__guest__txn_idvl_pty_cleansed'),
            join_columns=['data_txn_idvl_pty_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__guest__txn_idvl_pty_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_txn_idvl_pty_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_txn_idvl_pty_id']
    ) }}
),
...
final as (
    select
        ...
        data_sfx_cd,
        data_prmy_lang_cd,
        data_idvl_pty_actv_in,
        data_idvl_lst_nm,
        data_idvl_fst_nm,
        data_idvl_mid_nm,
        data_txn_idvl_pty_inactv_dts,
        data_age_nb,
        data_pos_lst_nm_cd,
        data_fvrt_chrctr_nm,
        data_dvc_mbr_typ_nm,
