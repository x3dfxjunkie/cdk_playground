{{
    config(
        ...
        unique_key = ['data_gro_nm','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__group_management__gro_cleansed'),
            join_columns=['data_gro_nm'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__group_management__gro_cleansed') }}
    {% if is_incremental() %}
