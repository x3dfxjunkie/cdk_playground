{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__op_ntfctn_cleansed'),
            join_columns=['data_op_ntfctn_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__op_ntfctn_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_op_ntfctn_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_op_ntfctn_id']
    ) }}
),
...
final as (
    select
        ...
        data_op_ntfctn_typ_short_ds,
        data_op_ntfctn_typ_full_ds,
        data_op_ntfctn_dismissed_in,
        data_op_ntfctn_read_in,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
