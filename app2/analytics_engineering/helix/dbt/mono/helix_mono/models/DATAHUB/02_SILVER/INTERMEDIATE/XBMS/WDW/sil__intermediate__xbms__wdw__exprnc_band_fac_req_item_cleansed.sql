{{
    config(
        ...
        unique_key= ['data_exprnc_band_fac_req_item_id','metadata_checksum'],
...
with source as (
    ...
    from {{ source('brz_xbms_wdw', 'exprnc_band_fac_req_item') }}
...
renamed as (
    select
        ...
        TRIM(src.data_dsny_prod_cn) as data_dsny_prod_cn,
    ...
    on src.data_exprnc_band_fac_req_item_id = min_source_update_datetime.data_exprnc_band_fac_req_item_id
),
...
final as (
    {{ dbt_utils.deduplicate(
        relation = 'renamed',
        partition_by = 'data_exprnc_band_fac_req_item_id, metadata_checksum',
