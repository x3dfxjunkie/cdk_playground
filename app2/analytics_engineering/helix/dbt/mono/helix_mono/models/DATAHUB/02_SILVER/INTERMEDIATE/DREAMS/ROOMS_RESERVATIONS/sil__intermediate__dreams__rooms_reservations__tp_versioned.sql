{{
    config(
        ...
        unique_key = ['data_tp_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_reservations__tp_cleansed'),
            join_columns=['data_tp_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_reservations__tp_cleansed') }}
    {% if is_incremental() %}
