{{
    config(
        ...
        unique_key = ['data_grp_cd', 'data_tc_grp_nb','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_reservations__grp_tc_grp_cleansed'),
            join_columns=['data_grp_cd', 'data_tc_grp_nb'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_reservations__grp_tc_grp_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_grp_cd, data_tc_grp_nb
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_grp_cd', 'data_tc_grp_nb']
    ) }}
),
