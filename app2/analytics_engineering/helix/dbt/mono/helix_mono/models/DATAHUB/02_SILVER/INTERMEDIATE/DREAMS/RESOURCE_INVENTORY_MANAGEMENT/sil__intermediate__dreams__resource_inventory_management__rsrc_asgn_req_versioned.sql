{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__resource_inventory_management__rsrc_asgn_req_cleansed'),
            join_columns=['data_rsrc_asgn_req_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__resource_inventory_management__rsrc_asgn_req_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_rsrc_asgn_req_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_rsrc_asgn_req_id']
    ) }}
),
...
final as (
    select
        ...
        data_asgn_ownr_id,
        data_asgn_req_dt,
        data_asgn_req_end_tm,
        data_asgn_req_strt_tm,
        data_create_dts,
