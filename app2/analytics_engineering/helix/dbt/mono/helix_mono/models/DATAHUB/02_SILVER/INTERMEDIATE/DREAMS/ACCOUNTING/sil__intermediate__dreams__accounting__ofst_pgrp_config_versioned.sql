{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__ofst_pgrp_config_cleansed'),
            join_columns=['data_ofst_pst_grp_config_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__ofst_pgrp_config_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_ofst_pst_grp_config_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_ofst_pst_grp_config_id']
    ) }}
),
...
final as (
    select
        ...
        data_ofst_pst_grp_id,
        data_ofst_pst_grp_config_strt_dt,
        data_ofst_pst_grp_config_end_dt,
        data_rev_cls_id,
        data_rev_typ_id,
        data_prod_id,
        data_pkg_id,
