{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__genie_constraint_cleansed'),
            join_columns=['data_constraint_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__genie_constraint_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_constraint_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_constraint_id']
    ) }}
),
...
final as (
    select
        data_constraint_id,
        data_cntnt_cache_id,
        data_constraint_type,
        data_constraint_values,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
