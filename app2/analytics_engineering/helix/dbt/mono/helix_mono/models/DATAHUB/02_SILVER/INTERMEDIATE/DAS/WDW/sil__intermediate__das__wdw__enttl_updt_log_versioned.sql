{{
    config(
        ...
        unique_key = ['data_log_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__das__wdw__enttl_updt_log_cleansed'),
            join_columns=['data_log_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__das__wdw__enttl_updt_log_cleansed') }}
    {% if is_incremental() %}
