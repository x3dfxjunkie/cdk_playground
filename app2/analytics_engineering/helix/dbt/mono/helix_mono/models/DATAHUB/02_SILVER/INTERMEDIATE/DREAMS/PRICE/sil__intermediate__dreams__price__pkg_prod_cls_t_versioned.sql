{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__pkg_prod_cls_t_cleansed'),
            join_columns=['data_pkg_prod_cls_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__pkg_prod_cls_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_pkg_prod_cls_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pkg_prod_cls_id']
    ) }}
),
...
final as (
    select
        ...
        data_jdo_seq_nb,
        data_pkg_id,
        data_pkg_prod_cls_cn,
        data_pkg_prod_cls_id,
        data_pkg_prod_cls_max_nb,
        data_pkg_prod_cls_min_nb,
        data_pkg_prod_cls_opt_in,
        data_pkg_prod_cls_slct_in,
        data_prge_dts,
        data_prod_cls_id,
        data_updt_dts,
        data_updt_usr_id_cd,
