{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__price_sheet_elmnt_t_cleansed'),
            join_columns=['data_price_sheet_elmnt_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__price_sheet_elmnt_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_price_sheet_elmnt_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_price_sheet_elmnt_id']
    ) }}
),
...
final as (
    select
        ...
        data_entrprs_price_id,
        data_jdo_seq_nb,
        data_prge_dts,
        data_price_sheet_elmnt_bkng_end_dts,
        data_price_sheet_elmnt_id,
        data_price_sheet_elmnt_usg_end_dts,
        data_price_sheet_elmnt_usg_strt_dts,
        data_price_sheet_elmt_bkng_strt_dts,
        data_price_sheet_id,
        data_rt_ctgy_cd,
        data_updt_dts,
        data_updt_usr_id_cd,
