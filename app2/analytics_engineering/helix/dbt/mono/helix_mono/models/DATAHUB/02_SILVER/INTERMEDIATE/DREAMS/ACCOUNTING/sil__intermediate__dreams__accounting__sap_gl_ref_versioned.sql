{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__sap_gl_ref_cleansed'),
            join_columns=['data_sap_gl_ref_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__sap_gl_ref_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_sap_gl_ref_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_sap_gl_ref_id']
    ) }}
),
...
final as (
    select
        ...
        data_sap_tax_cd,
        data_sap_wbs_cd,
        data_tax_exmpt_sap_gl_acct_nb,
        data_tax_exmpt_tax_cd,
