{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__bank_out_cleansed'),
            join_columns=['data_bank_out_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__bank_out_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_bank_out_id']
    ) }}
),
...
final as (
    select
        data_bank_out_id,
        data_bank_out_typ_nm,
        data_bank_in_id,
        data_bank_out_acct_dt,
        data_dpst_bag_id,
        data_bank_out_dts,
        data_bank_out_comp_dts,
        data_rgstr_am,
        data_rgstr_crncy_cd,
        data_tot_cash_am,
        data_tot_csh_crncy_cd,
        data_tot_non_csh_am,
        data_tot_non_csh_crncy_cd,
        data_tot_edc_am,
        data_tot_edc_crncy_cd,
