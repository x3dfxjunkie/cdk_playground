{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__dsny_fac_typ_cleansed'),
            join_columns=['data_dsny_fac_typ_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__dsny_fac_typ_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_dsny_fac_typ_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_dsny_fac_typ_id']
    ) }}
),
...
final as (
    select
        ...
        data_dsny_fac_typ_cd,
        data_dsny_fac_typ_ds,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
