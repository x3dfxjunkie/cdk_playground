{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__acct_dstr_chan_mtx_cleansed'),
            join_columns=['data_acct_dstr_chan_mtrx_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__acct_dstr_chan_mtx_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_acct_dstr_chan_mtrx_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_acct_dstr_chan_mtrx_id']
    ) }}
),
...
final as (
    select
        ...
        data_sls_chan_nm,
        data_comnctn_chan_nm,
        data_acct_dstr_chan_mtrx_strt_dt,
        data_acct_dstr_chan_mtrx_end_dt,
        data_acct_dstr_chan_mtrx_actv_in,
