{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__gbl_tkt_comm_cleansed'),
            join_columns=['data_glbl_tkt_comm_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__gbl_tkt_comm_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_glbl_tkt_comm_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_glbl_tkt_comm_id']
    ) }}
),
...
final as (
    select
        ...
        data_prod_id,
        data_tkt_cd,
        data_glbl_tkt_comm_pc,
        data_glbl_tkt_comm_strt_dt,
        data_glbl_tkt_comm_chg_dt,
        data_glbl_tkt_comm_rfrsh_dt,
