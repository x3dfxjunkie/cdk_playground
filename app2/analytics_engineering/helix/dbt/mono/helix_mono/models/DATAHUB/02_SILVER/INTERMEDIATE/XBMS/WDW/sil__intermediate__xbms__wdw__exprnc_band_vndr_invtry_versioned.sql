{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_vndr_invtry_cleansed'),
            join_columns=['data_exprnc_band_vndr_invtry_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_vndr_invtry_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_band_vndr_invtry_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_band_vndr_invtry_id']
    ) }}
),
...
final as (
    select
        ...
        data_vndr_band_reord_ntfctn_dt,
        data_invtry_on_hnd_cn,
        data_invtry_comtd_cn,
        data_invtry_avail_cn,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
