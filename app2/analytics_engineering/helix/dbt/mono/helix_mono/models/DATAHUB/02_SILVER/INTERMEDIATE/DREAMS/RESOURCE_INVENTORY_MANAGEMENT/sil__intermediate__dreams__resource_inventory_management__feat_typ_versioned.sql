{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__resource_inventory_management__feat_typ_cleansed'),
            join_columns=['data_feat_typ_nm'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__resource_inventory_management__feat_typ_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_feat_typ_nm']
    ) }}
),
...
final as (
    select
        ...
        data_feat_typ_ds,
        data_feat_typ_nm,
        data_feat_typ_req_nm,
        data_feat_typ_strt_dt,
        data_jdo_seq_nb,
        data_updt_dts,
        data_updt_usr_id_cd,
        data_feat_typ_end_dt,
        metadata_checksum,
        metadata_timestamp,
