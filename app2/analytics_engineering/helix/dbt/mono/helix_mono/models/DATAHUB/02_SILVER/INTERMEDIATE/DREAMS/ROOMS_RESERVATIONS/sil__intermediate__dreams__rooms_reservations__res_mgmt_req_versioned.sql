{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_reservations__res_mgmt_req_cleansed'),
            join_columns=['data_res_mgmt_req_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_reservations__res_mgmt_req_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_res_mgmt_req_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_res_mgmt_req_id']
    ) }}
),
...
final as (
    select
        ...
        data_res_mgmt_req_tx,
        data_res_mgmt_req_typ_nm,
        data_tps_id,
        data_tp_id,
        data_updt_dts,
        data_updt_usr_id_cd,
        data_tc_id,
        data_res_mgmt_prfl_id,
        data_res_mgmt_req_tx1,
        data_prge_dts,
        data_source,
        metadata_checksum,
        metadata_timestamp,
