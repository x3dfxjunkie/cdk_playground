{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__prod_lnk_t_cleansed'),
            join_columns=['data_prod_lnk_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__prod_lnk_t_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_prod_lnk_id']
    ) }}
),
...
final as (
    select
        data_entrprs_meal_prd_id,
        data_prge_dts,
        data_prmy_prod_id,
        data_prod_lnk_end_dt,
        data_prod_lnk_id,
        data_prod_lnk_strt_dt,
        data_prod_lnk_typ_nm,
        data_sec_prod_id,
        metadata_checksum,
        metadata_timestamp,
