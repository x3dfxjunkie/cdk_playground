{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__pmt_meth_typ_cleansed'),
            join_columns=['data_pmt_meth_typ_nm'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__pmt_meth_typ_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_pmt_meth_typ_nm
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pmt_meth_typ_nm']
    ) }}
),
...
final as (
    select
        ...
        data_pmt_meth_typ_cd,
        data_pmt_meth_typ_strt_dt,
        data_pmt_meth_typ_end_dt,
        data_auth_neg_am_in,
        data_dspl_seq_nb,
