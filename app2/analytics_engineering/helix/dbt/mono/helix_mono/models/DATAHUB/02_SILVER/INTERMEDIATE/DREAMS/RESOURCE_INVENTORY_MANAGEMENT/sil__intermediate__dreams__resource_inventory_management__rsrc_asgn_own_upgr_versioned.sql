{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__resource_inventory_management__rsrc_asgn_own_upgr_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__resource_inventory_management__rsrc_asgn_own_upgr_cleansed') }}
    {% if is_incremental() %}
