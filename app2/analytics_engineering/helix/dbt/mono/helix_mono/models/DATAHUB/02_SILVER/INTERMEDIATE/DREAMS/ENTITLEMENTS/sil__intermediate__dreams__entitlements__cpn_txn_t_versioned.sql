{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__entitlements__cpn_txn_t_cleansed'),
            join_columns=['data_cpn_chrg_id', 'data_orig_chrg_id', 'data_txn_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__entitlements__cpn_txn_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_cpn_chrg_id, data_orig_chrg_id, data_txn_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_cpn_chrg_id', 'data_orig_chrg_id', 'data_txn_id']
    ) }}
),
...
final as (
    select
        ...
        data_jdo_seq_nb,
        data_orig_chrg_folio_indicator,
        data_orig_chrg_id,
        data_prge_dts,
        data_txn_cmt_tx,
        data_txn_id,
        data_txn_proc_typ_nm,
        data_txn_rsn_cd,
        data_txn_typ_nm,
        data_updt_dts,
        data_updt_usr_id_cd,
