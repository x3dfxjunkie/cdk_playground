{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__xband_extnl_prod_cleansed'),
            join_columns=['data_xband_extnl_prod_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__xband_extnl_prod_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_xband_extnl_prod_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_xband_extnl_prod_id']
    ) }}
),
...
final as (
    select
        ...
        data_extnl_prod_src_sys_ntv_id,
        data_extnl_prod_txn_src_sys_tx,
        data_extnl_prod_strt_dts,
        data_extnl_prod_end_dts,
        data_extnl_prod_create_strt_dts,
        data_extnl_prod_create_end_dts,
        data_xband_enttl_prod_assc_actv_in,
        data_create_usr_id,
        data_create_dts,
        data_group_in,
        data_updt_usr_id,
