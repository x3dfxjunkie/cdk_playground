{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__fac_prod_t_cleansed'),
            join_columns=['data_fac_id', 'data_fac_prod_typ_nm', 'data_prod_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__fac_prod_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_fac_id, data_fac_prod_typ_nm, data_prod_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_fac_id', 'data_fac_prod_typ_nm', 'data_prod_id']
    ) }}
),
...
final as (
    select
        data_chk_in_loc_tx,
        data_fac_id,
        data_fac_prod_end_dt,
        data_fac_prod_strt_dt,
