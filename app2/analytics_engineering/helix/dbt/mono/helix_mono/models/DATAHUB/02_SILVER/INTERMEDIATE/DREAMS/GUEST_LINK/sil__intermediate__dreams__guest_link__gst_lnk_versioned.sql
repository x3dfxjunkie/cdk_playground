{{
    config(
        ...
        unique_key = ['data_gst_lnk_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__guest_link__gst_lnk_cleansed'),
            join_columns=['data_gst_lnk_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__guest_link__gst_lnk_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_gst_lnk_id']
    ) }}
),
