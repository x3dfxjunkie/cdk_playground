{{
    config(
        ...
        unique_key= ['data_acct_ctr_sap_id','metadata_checksum'],
...
with source as (
    ...
    from {{ source('brz_dreams_accounting', 'acct_ctr_sap') }}
...
renamed as (
    select
        ...
        src.data_acct_ctr_sap_end_dt as data_acct_ctr_sap_end_dt,
        src.data_acct_ctr_sap_id as data_acct_ctr_sap_id,
        src.data_acct_ctr_sap_strt_dt as data_acct_ctr_sap_strt_dt,
        ...
        TRIM(src.data_sap_div_cd) as data_sap_div_cd,
        TRIM(src.data_sap_dstr_chan_nm) as data_sap_dstr_chan_nm,
        TRIM(src.data_sap_prft_ctr_cd) as data_sap_prft_ctr_cd,
        TRIM(src.data_sap_sls_org_cd) as data_sap_sls_org_cd,
        TRIM(src.data_sls_ofc_id) as data_sls_ofc_id,
        ...
        {{ macro_generate_checksum([
            ...
            'src.data_acct_ctr_sap_end_dt',
            'src.data_acct_ctr_sap_strt_dt',
            'src.data_create_dts',
    ...
    on src.data_acct_ctr_sap_id = min_source_update_datetime.data_acct_ctr_sap_id
),
...
final as (
    {{ dbt_utils.deduplicate(
        relation = 'renamed',
        partition_by = 'data_acct_ctr_sap_id, metadata_checksum',
