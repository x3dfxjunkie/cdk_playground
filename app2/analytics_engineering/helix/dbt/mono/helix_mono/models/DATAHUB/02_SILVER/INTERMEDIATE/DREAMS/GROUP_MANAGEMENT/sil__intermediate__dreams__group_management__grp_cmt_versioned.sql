{{
    config(
        ...
        unique_key = ['data_grp_cmt_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__group_management__grp_cmt_cleansed'),
            join_columns=['data_grp_cmt_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__group_management__grp_cmt_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_grp_cmt_id']
    ) }}
),
