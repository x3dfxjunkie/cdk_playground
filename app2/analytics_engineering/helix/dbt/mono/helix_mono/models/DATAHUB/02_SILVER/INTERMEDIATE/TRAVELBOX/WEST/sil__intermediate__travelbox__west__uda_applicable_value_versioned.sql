{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__west__uda_applicable_value_cleansed'),
            join_columns=['data_applicable_value_id', 'data_attribute_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__west__uda_applicable_value_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_applicable_value_id, data_attribute_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_applicable_value_id', 'data_attribute_id']
    ) }}
),
...
final as (
    select
        data_applicable_value_code,
        data_applicable_value_id,
        data_attribute_id,
        data_applicable_values,
        data_default,
        data_attribute_type,
        data_active,
