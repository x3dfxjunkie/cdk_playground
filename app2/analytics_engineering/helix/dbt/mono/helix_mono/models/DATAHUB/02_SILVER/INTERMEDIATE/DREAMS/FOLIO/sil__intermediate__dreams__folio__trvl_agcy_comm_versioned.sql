{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__trvl_agcy_comm_cleansed'),
            join_columns=['data_trvl_agcy_comm_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__trvl_agcy_comm_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_trvl_agcy_comm_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_trvl_agcy_comm_id']
    ) }}
),
...
final as (
    select
        data_chrg_grp_chkin_dt,
        data_chrg_grp_chkot_dt,
        data_cmps_id,
        data_comm_apprvl_dt,
        data_comm_ap_intfc_id,
        data_comm_pst_dts,
        data_comm_sts_nm,
        data_create_dts,
        data_create_usr_id_cd,
        data_fac_id,
        data_res_rspb_pty_id,
