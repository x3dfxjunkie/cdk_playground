{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__fac_req_shipmt_cnfirm_cleansed'),
            join_columns=['data_fac_req_shipmt_cnfirm_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__fac_req_shipmt_cnfirm_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_fac_req_shipmt_cnfirm_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_fac_req_shipmt_cnfirm_id']
    ) }}
),
...
final as (
    select
        ...
        data_fac_shipmt_correct_loc_in,
