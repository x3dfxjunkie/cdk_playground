{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__evt_adt_log_cleansed'),
            join_columns=['data_evt_adt_log_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__evt_adt_log_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_evt_adt_log_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_evt_adt_log_id']
    ) }}
),
...
final as (
    select
        ...
        data_evt_typ_tx,
        data_evt_gst_id,
        data_evt_correlation_id,
        data_evt_payload_tx,
        data_evt_timestamp,
        data_evt_prmy_acknwl_sys_nm,
        data_evt_sec_acknwl_sys_nm,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
