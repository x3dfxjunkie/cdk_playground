{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__prod_chrg_cleansed'),
            join_columns=['data_chrg_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__prod_chrg_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_chrg_id']
    ) }}
),
...
final as (
    select
        ...
        data_jdo_seq_nb,
        data_ovrd_prod_chrg_in,
        data_prge_dts,
        data_prod_cd,
        data_prod_chrg_upgrd_typ_nm,
        data_prod_ds,
        data_prod_id,
        data_prod_typ_nm,
        data_sls_chan_nm,
