{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__guest_link__txn_gst_xref_cleansed'),
            join_columns=['data_gst_lnk_txn_gst_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__guest_link__txn_gst_xref_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_gst_lnk_txn_gst_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_gst_lnk_txn_gst_id']
    ) }}
),
...
final as (
    select
        data_txn_gst_xref_src_nm,
        data_gst_lnk_txn_gst_id,
        data_txn_gst_xref_vl,
