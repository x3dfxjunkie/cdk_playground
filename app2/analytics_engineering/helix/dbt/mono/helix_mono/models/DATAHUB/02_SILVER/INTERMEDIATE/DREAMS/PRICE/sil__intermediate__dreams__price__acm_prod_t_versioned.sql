{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__acm_prod_t_cleansed'),
            join_columns=['data_acm_prod_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__acm_prod_t_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_acm_prod_id']
    ) }}
),
...
final as (
    select
        data_acm_prod_id,
        data_rm_typ_cd,
        data_acm_prod_addl_chg_thrshld_nb,
        data_prge_dts,
        metadata_checksum,
        metadata_timestamp,
