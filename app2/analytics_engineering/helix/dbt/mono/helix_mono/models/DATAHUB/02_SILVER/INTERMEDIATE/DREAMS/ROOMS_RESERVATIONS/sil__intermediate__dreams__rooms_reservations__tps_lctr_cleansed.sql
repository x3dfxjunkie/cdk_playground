{{
    config(
        ...
        unique_key= ['data_tps_id', 'data_tps_lctr_id','metadata_checksum'],
...
with source as (
    ...
    from {{ source('brz_dreams_rooms_reservations', 'tps_lctr') }}
...
renamed as (
    select
        ...
        src.data_tps_lctr_id as data_tps_lctr_id,
        TRIM(src.data_lctr_typ_nm) as data_lctr_typ_nm,
        TRIM(src.data_lctr_vl) as data_lctr_vl,
    ...
    and src.data_tps_lctr_id = min_source_update_datetime.data_tps_lctr_id
),
...
final as (
    {{ dbt_utils.deduplicate(
        relation = 'renamed',
        partition_by = 'data_tps_id, data_tps_lctr_id, metadata_checksum',
