{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_reservations__acm_cmpnt_chrnlgy_cleansed'),
            join_columns=['data_acm_cmpnt_chrnlgy_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_reservations__acm_cmpnt_chrnlgy_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_acm_cmpnt_chrnlgy_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_acm_cmpnt_chrnlgy_id']
    ) }}
),
...
final as (
    select
        ...
        data_chrnlgcl_evt_typ_nm,
        data_chrnlgcl_evt_src_nm,
        data_acm_cmpnt_dts,
        metadata_checksum,
        metadata_timestamp,
