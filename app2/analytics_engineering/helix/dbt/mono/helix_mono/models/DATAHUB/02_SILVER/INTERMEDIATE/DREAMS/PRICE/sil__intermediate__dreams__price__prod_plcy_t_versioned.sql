{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__prod_plcy_t_cleansed'),
            join_columns=['data_plcy_id', 'data_prod_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__prod_plcy_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_plcy_id, data_prod_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_plcy_id', 'data_prod_id']
    ) }}
),
...
final as (
    select
        ...
        data_prge_dts,
        data_prod_id,
        data_prod_plcy_end_dt,
        data_prod_plcy_strt_dt,
        data_updt_dts,
        metadata_checksum,
        metadata_timestamp,
