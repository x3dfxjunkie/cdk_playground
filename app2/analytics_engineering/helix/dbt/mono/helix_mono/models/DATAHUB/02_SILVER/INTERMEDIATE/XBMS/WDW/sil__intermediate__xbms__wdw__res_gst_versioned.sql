{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__res_gst_cleansed'),
            join_columns=['data_gst_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__res_gst_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        ...
        data_logical_del_in,
        data_ap_exp_dt,
        data_gst_enttl_ovrd_in,
        data_dvc_mbr_in,
        data_ap_tkt_id,
        data_ap_tkt_cd,
        data_ap_tkt_config_ds,
        data_actv_gst_in,
        data_gst_frst_nm_upper,
        data_gst_lst_nm_upper,
        data_incog_req_id,
        metadata_checksum,
        metadata_timestamp,
