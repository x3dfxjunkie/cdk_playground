...
-- define a cte for tpel
tpel_cte as (
    select
        *
    from
        {{ ref('sil__intermediate__dreams__guest__txn_pty_eml_lctr_versioned') }}
    where
        metadata_version_end_datetime = '{{ var('metadata_version_end_date') }}'
        and upper(metadata_operation) != 'DELETE'
),
...
final as (
    select
        ...
        tpel.data_txn_pty_eml_addr_tx as transaction_party_email_address,
        ...
        tpel.data_allow_eml_in as allow_email_indicator
    from
        tp_cte tp
        ...
        inner join tpel_cte tpel
        on tpl.data_txn_pty_lctr_id = tpel.data_txn_pty_eml_lctr_id
