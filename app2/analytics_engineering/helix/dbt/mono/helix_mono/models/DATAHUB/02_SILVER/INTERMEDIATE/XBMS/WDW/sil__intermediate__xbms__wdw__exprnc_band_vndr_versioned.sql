{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_vndr_cleansed'),
            join_columns=['data_exprnc_band_vndr_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_vndr_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_band_vndr_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_band_vndr_id']
    ) }}
),
...
final as (
    select
        ...
        data_ship_ctct_id,
        data_addr_id,
        data_exprnc_band_vndr_nm,
        data_exprnc_band_vndr_cd,
        data_exprnc_band_vndr_avail_po_in,
        data_sap_id,
        data_gst_ship_strt_ti,
        data_gst_ship_end_ti,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_logical_del_in,
        data_plant_cd,
        data_exprnc_band_vndr_avail_gi_in,
        metadata_checksum,
        metadata_timestamp,
