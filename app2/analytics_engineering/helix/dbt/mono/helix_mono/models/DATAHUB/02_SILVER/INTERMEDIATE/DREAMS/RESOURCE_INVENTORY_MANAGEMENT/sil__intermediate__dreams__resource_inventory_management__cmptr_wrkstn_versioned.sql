{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__resource_inventory_management__cmptr_wrkstn_cleansed'),
            join_columns=['data_cmptr_wrkstn_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__resource_inventory_management__cmptr_wrkstn_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_cmptr_wrkstn_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_cmptr_wrkstn_id']
    ) }}
),
...
final as (
    select
        ...
        data_cmptr_wrkstn_ip_addr_tx,
        data_cmptr_wrkstn_nm,
        data_create_dts,
