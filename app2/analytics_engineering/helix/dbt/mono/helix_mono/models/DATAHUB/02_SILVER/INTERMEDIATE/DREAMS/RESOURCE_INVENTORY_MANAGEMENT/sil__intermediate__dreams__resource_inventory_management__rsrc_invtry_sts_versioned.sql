{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__resource_inventory_management__rsrc_invtry_sts_cleansed'),
            join_columns=['data_rsrc_invtry_sts_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__resource_inventory_management__rsrc_invtry_sts_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_rsrc_invtry_sts_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_rsrc_invtry_sts_id']
    ) }}
),
...
final as (
    select
        ...
        data_rsrc_invtry_sts_strt_dts,
        data_rsrc_invtry_sts_typ_nm,
        data_sts_ds,
        data_sts_nm,
        data_updt_dts,
        data_updt_usr_id_cd,
        data_rsrc_invtry_sts_end_dts,
        metadata_checksum,
        metadata_timestamp,
