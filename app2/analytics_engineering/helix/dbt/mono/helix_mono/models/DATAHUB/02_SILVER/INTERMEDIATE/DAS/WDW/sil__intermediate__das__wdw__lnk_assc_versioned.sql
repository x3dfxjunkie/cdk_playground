{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__das__wdw__lnk_assc_cleansed'),
            join_columns=['data_actv_lnk_id', 'data_mergd_lnk_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__das__wdw__lnk_assc_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_actv_lnk_id, data_mergd_lnk_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_actv_lnk_id', 'data_mergd_lnk_id']
    ) }}
),
