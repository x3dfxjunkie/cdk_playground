{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__res_seg_cleansed'),
            join_columns=['data_res_seg_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__res_seg_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_res_seg_id']
    ) }}
),
...
final as (
    select
        ...
        data_updt_dts,
        data_logical_del_in,
        data_trvl_plan_seg_id,
        data_trvl_cmpnt_id,
        data_res_id,
        data_trvl_cmpnt_sts_id,
        data_grp_mstr_rec_tm_id,
        data_res_seg_pkg_cd,
        data_res_seg_pkg_prod_id,
        data_res_seg_pkg_ds,
        data_res_seg_spcl_offer_in,
        metadata_checksum,
        metadata_timestamp,
