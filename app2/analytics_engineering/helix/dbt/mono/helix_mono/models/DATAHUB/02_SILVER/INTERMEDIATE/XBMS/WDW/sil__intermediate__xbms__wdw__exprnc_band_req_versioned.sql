{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_req_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_req_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        ...
        data_updt_dts,
        data_logical_del_in,
        data_exprnc_band_addr_confirmed_in,
        data_curr_exprnc_band_req_sts_id,
        data_exprnc_band_req_sts_ovrd_in,
        metadata_checksum,
        metadata_timestamp,
