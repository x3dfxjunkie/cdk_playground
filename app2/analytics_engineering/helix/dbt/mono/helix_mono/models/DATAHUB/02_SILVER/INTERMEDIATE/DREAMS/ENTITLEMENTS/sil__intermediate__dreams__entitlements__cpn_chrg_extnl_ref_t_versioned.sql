{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__entitlements__cpn_chrg_extnl_ref_t_cleansed'),
            join_columns=['data_chrg_extnl_ref_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__entitlements__cpn_chrg_extnl_ref_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_chrg_extnl_ref_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_chrg_extnl_ref_id']
    ) }}
),
