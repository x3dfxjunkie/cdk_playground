{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__pmt_meth_cleansed'),
            join_columns=['data_pmt_meth_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__pmt_meth_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pmt_meth_id']
    ) }}
),
...
final as (
    select
        ...
        data_pmt_meth_cd,
        data_rfnd_ovrd_pmt_meth_in,
        data_max_allw_am,
        data_max_allw_crncy_cd,
        data_strts_pmt_auth_cd,
