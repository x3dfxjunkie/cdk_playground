{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__chrg_alloc_cleansed'),
            join_columns=['data_chrg_alloc_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__chrg_alloc_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_chrg_alloc_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_chrg_alloc_id']
    ) }}
),
...
final as (
    select
        data_chrg_alloc_id,
        data_own_chrg_grp_id,
        data_chrg_alloc_typ_nm,
        data_pick_chrg_grp_id,
        data_bill_chrg_alloc_in,
        data_splt_typ_nm,
        data_splt_vl,
        data_asgn_seq_nb,
        data_txn_fac_id,
        ...
        data_updt_dts,
        data_jdo_seq_nb,
        data_bill_chrg_grp_id,
        data_txn_idvl_pty_id,
        data_alloc_strt_dt,
        data_alloc_end_dt,
        data_rev_cls_id,
        data_chrg_alloc_inactv_dt,
        data_bill_id,
        data_appr_by_nm,
        data_folio_prfl_id,
        data_prge_dts,
