{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__west__cli_payment_group_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__west__cli_payment_group_cleansed') }}
    {% if is_incremental() %}
