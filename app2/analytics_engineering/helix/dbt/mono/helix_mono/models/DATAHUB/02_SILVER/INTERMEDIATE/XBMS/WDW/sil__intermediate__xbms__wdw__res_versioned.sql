{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__res_cleansed'),
            join_columns=['data_res_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__res_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        ...
        data_logical_del_in,
        data_txn_src_sys_tx,
        data_alt_txn_src_sys_id,
        data_alt_txn_src_sys_tx,
        data_exprnc_begin_dt,
        data_frst_dsny_fac_id,
        data_frst_trvl_plan_seg_id,
        data_frst_trvl_cmpnt_id,
        data_frst_grp_mst_rec_tm_id,
        metadata_checksum,
        metadata_timestamp,
