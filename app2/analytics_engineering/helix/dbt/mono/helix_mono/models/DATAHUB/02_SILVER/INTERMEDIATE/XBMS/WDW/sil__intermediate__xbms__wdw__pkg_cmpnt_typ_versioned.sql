{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__pkg_cmpnt_typ_cleansed'),
            join_columns=['data_pkg_cmpnt_typ_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__pkg_cmpnt_typ_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_pkg_cmpnt_typ_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pkg_cmpnt_typ_id']
    ) }}
),
...
final as (
    select
        ...
        data_pkg_cmpnt_typ_nm,
        data_pkg_cmpnt_typ_actv_in,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_logical_del_in,
        data_pkg_cmpnt_addl_item_in,
        data_pkg_cmpnt_typ_rtl_in,
        metadata_checksum,
        metadata_timestamp,
