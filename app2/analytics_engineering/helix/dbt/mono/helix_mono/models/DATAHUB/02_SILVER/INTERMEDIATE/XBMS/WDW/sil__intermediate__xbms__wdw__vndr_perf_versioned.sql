{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__vndr_perf_cleansed'),
            join_columns=['data_vndr_perf_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__vndr_perf_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_vndr_perf_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_vndr_perf_id']
    ) }}
),
...
final as (
    select
        ...
        data_exprnc_band_fflmt_vndr_id,
        data_vndr_lead_tm_di,
        data_vndr_lead_tm_std_dvt_di,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
