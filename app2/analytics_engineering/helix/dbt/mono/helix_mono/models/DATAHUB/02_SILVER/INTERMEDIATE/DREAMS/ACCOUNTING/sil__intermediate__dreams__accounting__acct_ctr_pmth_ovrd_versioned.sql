{{
    config(
        ...
        unique_key = ['data_acct_ctr_pmt_meth_id', 'data_ovrd_rfnd_pmt_meth_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__acct_ctr_pmth_ovrd_cleansed'),
            join_columns=['data_acct_ctr_pmt_meth_id', 'data_ovrd_rfnd_pmt_meth_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__acct_ctr_pmth_ovrd_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_acct_ctr_pmt_meth_id, data_ovrd_rfnd_pmt_meth_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_acct_ctr_pmt_meth_id', 'data_ovrd_rfnd_pmt_meth_id']
    ) }}
),
