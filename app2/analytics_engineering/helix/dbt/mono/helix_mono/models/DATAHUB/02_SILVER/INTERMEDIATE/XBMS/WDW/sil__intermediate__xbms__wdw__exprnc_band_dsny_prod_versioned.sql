{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_dsny_prod_cleansed'),
            join_columns=['data_exprnc_band_dsny_prod_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_dsny_prod_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_band_dsny_prod_id']
    ) }}
),
...
final as (
    select
        ...
        data_exprnc_band_top_color_id,
        data_exprnc_band_bottom_color_id,
        data_exprnc_band_erlst_fflmt_dy_cn,
        data_exprnc_band_shipmt_to_rsrt_in,
        data_exprnc_band_prod_avail_fr_dts,
        data_exprnc_band_prod_avail_to_dts,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_logical_del_in,
        data_avail_to_gst_ord_extnl_part_in,
        data_dflt_for_grp_in,
        data_avail_to_reord_extnl_part_in,
        data_avail_to_gmr_extnl_part_in,
        metadata_checksum,
        metadata_timestamp,
