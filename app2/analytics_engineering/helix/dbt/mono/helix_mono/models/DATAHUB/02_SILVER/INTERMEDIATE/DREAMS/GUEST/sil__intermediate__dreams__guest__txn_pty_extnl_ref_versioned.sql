{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__guest__txn_pty_extnl_ref_cleansed'),
            join_columns=['data_txn_pty_extnl_ref_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__guest__txn_pty_extnl_ref_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_txn_pty_extnl_ref_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_txn_pty_extnl_ref_id']
    ) }}
),
...
final as (
    select
        ...
        data_jdo_seq_nb,
        data_pty_extnl_src_nm,
        data_txn_pty_extnl_ref_id,
        data_txn_pty_extnl_ref_val,
        data_txn_pty_id,
        data_updt_dts,
        data_updt_usr_id_cd,
        metadata_checksum,
