{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__hawkeye_asin_sku_cleansed'),
            join_columns=['data_hawkeye_asin_sku_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__hawkeye_asin_sku_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_hawkeye_asin_sku_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_hawkeye_asin_sku_id']
    ) }}
),
...
final as (
    select
        ...
        data_hawkeye_asin_sku_cd,
        data_hawkeye_ble_sku_cd,
        data_hawkeye_ip_upc_cd,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
