{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__price_def_t_cleansed'),
            join_columns=['data_price_def_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__price_def_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_price_def_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_price_def_id']
    ) }}
),
...
final as (
    select
        ...
        data_jdo_seq_nb,
        data_prge_dts,
        data_price_def_age_am,
        data_price_def_disc_am,
        data_price_def_id,
        data_price_def_price_am,
        data_price_def_tax_am,
        data_price_def_tax_in,
        data_price_def_tkt_in,
