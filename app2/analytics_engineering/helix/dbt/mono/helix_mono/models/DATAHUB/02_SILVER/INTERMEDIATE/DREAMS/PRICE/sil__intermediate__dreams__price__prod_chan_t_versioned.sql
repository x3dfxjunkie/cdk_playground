{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__prod_chan_t_cleansed'),
            join_columns=['data_prod_chan_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__prod_chan_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_prod_chan_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_prod_chan_id']
    ) }}
),
...
final as (
    select
        ...
        data_entrprs_id,
        data_jdo_seq_nb,
        data_pkgr_prod_chan_id,
        data_pkg_chan_cd,
        data_prge_dts,
        data_prod_chan_actv_in,
        data_prod_chan_end_dt,
        data_prod_chan_id,
        data_prod_chan_nm,
        data_prod_chan_rsr_in,
        data_prod_chan_strt_dt,
        data_prod_chan_typ_nm,
        data_updt_dts,
        metadata_checksum,
        metadata_timestamp,
