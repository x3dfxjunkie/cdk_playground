{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_ord_sts_ctgy_cleansed'),
            join_columns=['data_exprnc_band_ord_sts_ctgy_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_ord_sts_ctgy_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_band_ord_sts_ctgy_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_band_ord_sts_ctgy_id']
    ) }}
),
...
final as (
    select
        ...
        data_exprnc_band_ord_sts_ctgy_cd,
        data_exprnc_band_ord_sts_ctgy_ds,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
