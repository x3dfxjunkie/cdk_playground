{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__lvl_n_enttl_typ_site_assc_cleansed'),
            join_columns=['data_lvl_n_enttl_typ_site_assc_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__lvl_n_enttl_typ_site_assc_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_lvl_n_enttl_typ_site_assc_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_lvl_n_enttl_typ_site_assc_id']
    ) }}
),
...
final as (
    select
        ...
        data_site,
        data_lvl_n_enttl_typ_site_assc_ds,
        data_create_use_id,
        data_create_use_dts,
        data_updt_usr_id,
        data_updt_dts,
