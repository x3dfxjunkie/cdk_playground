{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__ship_ctct_cleansed'),
            join_columns=['data_ship_ctct_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__ship_ctct_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_ship_ctct_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_ship_ctct_id']
    ) }}
),
...
final as (
    select
        data_ship_ctct_id,
        data_ship_ctct_full_nm,
        data_ship_ctct_ttl_nm,
        data_ship_ctct_eml_addr_tx,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
