{{
    config(
        ...
        unique_key= ['data_cpn_prfl_id','metadata_checksum'],
...
with source as (
    ...
    from {{ source('brz_dreams_entitlements', 'cpn_prfl_t') }}
...
renamed as (
    select
        ...
        TRIM(src.data_cpn_rdmpt_typ_nm) as data_cpn_rdmpt_typ_nm,
        TRIM(src.data_cpn_cd) as data_cpn_cd,
        TRIM(src.data_cpn_ds) as data_cpn_ds,
        TRIM(src.data_cpn_shrt_ds) as data_cpn_shrt_ds,
        ...
        {{ macro_generate_checksum([
            'src.data_cpn_rdmpt_typ_nm',
            'src.data_cpn_cd',
            'src.data_cpn_ds',
            'src.data_cpn_shrt_ds',
    ...
    on src.data_cpn_prfl_id = min_source_update_datetime.data_cpn_prfl_id
),
...
final as (
    {{ dbt_utils.deduplicate(
        relation = 'renamed',
        partition_by = 'data_cpn_prfl_id, metadata_checksum',
