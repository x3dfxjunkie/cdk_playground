{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__entitlements__enttl_pln_t_cleansed'),
            join_columns=['data_enttl_pln_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__entitlements__enttl_pln_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_enttl_pln_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_enttl_pln_id']
    ) }}
),
...
final as (
    select
        ...
        data_enttl_pln_ds,
        data_enttl_pln_end_dt,
        data_enttl_pln_id,
        data_enttl_pln_strt_dt,
        data_incognito,
        data_jdo_seq_nb,
        data_prge_dts,
