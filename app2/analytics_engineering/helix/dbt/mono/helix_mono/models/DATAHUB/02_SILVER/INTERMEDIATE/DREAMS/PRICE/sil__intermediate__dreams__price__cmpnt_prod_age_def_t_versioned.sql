{{
    config(
        ...
        unique_key = ['data_age_def_id', 'data_prod_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__cmpnt_prod_age_def_t_cleansed'),
            join_columns=['data_age_def_id', 'data_prod_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__cmpnt_prod_age_def_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_age_def_id, data_prod_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_age_def_id', 'data_prod_id']
    ) }}
),
