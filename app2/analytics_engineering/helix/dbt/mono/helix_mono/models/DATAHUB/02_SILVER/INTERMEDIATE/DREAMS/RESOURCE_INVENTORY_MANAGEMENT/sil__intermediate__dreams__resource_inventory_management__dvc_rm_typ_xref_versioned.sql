{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__resource_inventory_management__dvc_rm_typ_xref_cleansed'),
            join_columns=['data_dvc_rm_typ_xref_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__resource_inventory_management__dvc_rm_typ_xref_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_dvc_rm_typ_xref_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_dvc_rm_typ_xref_id']
    ) }}
),
...
final as (
    select
        ...
        data_dvc_rsrc_invtry_typ_cd,
        data_dvc_rsrt_fac_id,
        data_rsrc_invtry_typ_cd,
        metadata_checksum,
        metadata_timestamp,
