{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__ta_comm_rt_spec_cleansed'),
            join_columns=['data_comm_rt_spec_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__ta_comm_rt_spec_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_comm_rt_spec_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_comm_rt_spec_id']
    ) }}
),
...
final as (
    select
        ...
        data_rate_typ_nm,
        data_ta_comm_rt_vl,
        data_comm_spec_create_dt,
        data_comm_spec_strt_dt,
        data_comm_spec_end_dt,
        data_trvl_agcy_comm_ds,
        data_rev_cls_id,
        data_pkg_id,
        data_all_rev_cls_in,
        data_all_pkg_in,
