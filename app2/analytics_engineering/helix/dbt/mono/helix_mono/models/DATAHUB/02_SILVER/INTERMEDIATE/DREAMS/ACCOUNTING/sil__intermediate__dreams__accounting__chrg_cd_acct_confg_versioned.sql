{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__chrg_cd_acct_confg_cleansed'),
            join_columns=['data_chrg_cd_acct_config_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__chrg_cd_acct_confg_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_chrg_cd_acct_config_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_chrg_cd_acct_config_id']
    ) }}
),
...
final as (
    select
        ...
        data_non_csh_cd,
        data_ord_nb,
        data_chrg_cd_acct_cfg_strt_dt,
        data_chrg_cd_acct_cfg_end_dt,
        metadata_checksum,
        metadata_timestamp,
