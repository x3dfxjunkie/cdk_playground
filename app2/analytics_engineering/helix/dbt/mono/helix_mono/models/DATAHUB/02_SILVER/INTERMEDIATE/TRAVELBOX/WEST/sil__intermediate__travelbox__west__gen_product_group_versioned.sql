{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__west__gen_product_group_cleansed'),
            join_columns=['data_product_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__west__gen_product_group_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_product_id']
    ) }}
),
...
final as (
    select
        ...
        data_name,
        data_prebooked,
        data_voucher_per_day,
        data_multiple,
        data_exclude_from_normal_search,
        data_gp_lite,
        data_invoice_section,
        data_ancillary,
        metadata_checksum,
        metadata_timestamp,
