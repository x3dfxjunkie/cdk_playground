{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__fnc_typ_cleansed'),
            join_columns=['data_fnc_typ_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__fnc_typ_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_fnc_typ_id']
    ) }}
),
...
final as (
    select
        ...
        data_fnc_typ_nm,
        data_fnc_typ_cd,
        data_fnc_typ_strt_dt,
        data_fnc_typ_end_dt,
        data_fnc_typ_dspl_seq_nb,
