{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__profile__prfl_rte_cleansed'),
            join_columns=['data_prfl_rte_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__profile__prfl_rte_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_prfl_rte_id']
    ) }}
),
...
final as (
    select
        ...
        data_dflt_prfl_rte_in,
        data_kodo_prfl_rte_typ_nm,
