{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_enttl_cleansed'),
            join_columns=['data_exprnc_band_enttl_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_enttl_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_band_enttl_id']
    ) }}
),
...
final as (
    select
        ...
        data_skip_cust_sel_in,
        data_avail_prem_sel_in,
        metadata_checksum,
        metadata_timestamp,
