{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__ta_acm_comm_cleansed'),
            join_columns=['data_trvl_agcy_acm_comm_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__ta_acm_comm_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_trvl_agcy_acm_comm_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_trvl_agcy_acm_comm_id']
    ) }}
),
...
final as (
    select
        ...
        data_fac_id,
        data_rm_id_vl,
        data_acm_comm_strt_dt,
        data_acm_comm_end_dt,
