{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__acct_ctr_pmt_meth_cleansed'),
            join_columns=['data_acct_ctr_pmt_meth_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__acct_ctr_pmt_meth_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_acct_ctr_pmt_meth_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_acct_ctr_pmt_meth_id']
    ) }}
),
...
final as (
    select
        ...
        data_rfnd_dly_dy_cn,
        data_min_sty_dy_cn,
        data_cr_wroff_max_am,
        data_cr_wroff_max_crncy_cd,
        data_bank_in_proc_req_in,
        data_till_req_in,
