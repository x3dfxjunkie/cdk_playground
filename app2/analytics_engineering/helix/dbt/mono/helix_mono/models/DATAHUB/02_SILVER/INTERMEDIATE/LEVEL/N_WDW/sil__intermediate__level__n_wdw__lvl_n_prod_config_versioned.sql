{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__lvl_n_prod_config_cleansed'),
            join_columns=['data_lvl_n_prod_config_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__lvl_n_prod_config_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_lvl_n_prod_config_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_lvl_n_prod_config_id']
    ) }}
),
...
final as (
    select
        ...
        data_lvl_n_config_strt_dy_nb,
        data_lvl_n_config_end_dy_nb,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
