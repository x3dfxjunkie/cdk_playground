{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__shipmt_ntc_cleansed'),
            join_columns=['data_shipmt_ntc_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__shipmt_ntc_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_shipmt_ntc_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_shipmt_ntc_id']
    ) }}
),
...
final as (
    select
        ...
        data_shipmt_nb,
        data_po_id,
        data_shipmt_ntc_ship_dt,
        data_shipmt_ntc_expect_dlvr_dt,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_logical_del_in,
        data_adv_shipmt_ntfctn_file_nm,
