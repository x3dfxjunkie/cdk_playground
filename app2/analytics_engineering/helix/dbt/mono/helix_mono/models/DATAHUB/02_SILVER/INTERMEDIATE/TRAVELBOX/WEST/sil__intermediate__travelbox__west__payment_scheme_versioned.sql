{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__west__payment_scheme_cleansed'),
            join_columns=['data_pay_scheme_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__west__payment_scheme_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_pay_scheme_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pay_scheme_id']
    ) }}
),
...
final as (
    select
        data_pre_amount,
        data_pre_amount_type,
        data_pre_currency,
        data_post_payment_method,
        data_post_payment_type,
        data_post_payment_month_no,
        data_post_date,
        data_inv_due_period,
        data_checkout_date_consider,
        data_auto_payment,
        data_interface_code,
        data_depo_aft_bkg_bef_dept,
        data_bal_aft_bkg_bef_dept,
        data_payment_media,
        data_pay_scheme_id,
        data_payment_ref,
        data_description,
        data_product,
        data_pre_payment_method,
        data_pre_payment_type,
        data_pre_date,
        metadata_checksum,
        metadata_timestamp,
