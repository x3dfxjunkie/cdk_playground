{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__revenue_recognition_audit_cleansed'),
            join_columns=['data_transaction_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__revenue_recognition_audit_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_transaction_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_transaction_id']
    ) }}
),
...
final as (
    select
        ...
        data_revenue_total,
        data_site,
        data_def_revenue_total,
        data_status,
        data_response_code,
        data_payload,
        data_response_payload,
        data_create_dts,
        data_updt_dts,
        metadata_checksum,
        metadata_timestamp,
