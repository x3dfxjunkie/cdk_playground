{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__charge_account__chrg_grp_lnk_cleansed'),
            join_columns=['data_chrg_grp_lnk_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__charge_account__chrg_grp_lnk_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_chrg_grp_lnk_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_chrg_grp_lnk_id']
    ) }}
),
...
final as (
    select
        data_chrg_grp_lnk_dts,
        data_chrg_grp_lnk_id,
        data_chrg_grp_lnk_typ_nm,
        data_create_dts,
        data_create_usr_id_cd,
        data_jdo_seq_nb,
        data_lnkd_chrg_grp_id,
        data_lnkg_chrg_grp_id,
        data_rlvnt_chrg_grp_id,
        data_updt_dts,
        data_updt_usr_id_cd,
        metadata_checksum,
        metadata_timestamp,
