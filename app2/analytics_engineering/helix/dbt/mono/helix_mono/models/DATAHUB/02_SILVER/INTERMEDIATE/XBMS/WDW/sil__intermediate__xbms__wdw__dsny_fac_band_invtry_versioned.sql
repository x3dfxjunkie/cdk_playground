{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__dsny_fac_band_invtry_cleansed'),
            join_columns=['data_dsny_fac_band_invtry_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__dsny_fac_band_invtry_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_dsny_fac_band_invtry_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_dsny_fac_band_invtry_id']
    ) }}
),
...
final as (
    select
        ...
        data_exprnc_band_frst_rec_fac_dts,
        data_exprnc_band_rec_lvg_fac_dts,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_logical_del_in,
        data_fac_shipmt_case_nb,
        metadata_checksum,
        metadata_timestamp,
