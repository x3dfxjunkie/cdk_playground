{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__east__supplier_cleansed'),
            join_columns=['data_supplier_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__east__supplier_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_supplier_id']
    ) }}
),
...
final as (
    select
        ...
        data_cancel_scheme,
        data_commission,
        data_attended_person,
        data_email,
        data_fax,
        data_telephone,
        data_default_contact_method,
        data_send_immediately,
        data_web_url,
        data_preference,
        data_enable,
        data_payment_type,
        ...
        data_chain,
        data_locale,
        data_max_concurrent_user_count,
        data_dedup_order,
