{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__chrg_cd_infc_smry_cleansed'),
            join_columns=['data_chrg_cd_infc_smry_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__chrg_cd_infc_smry_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_chrg_cd_infc_smry_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_chrg_cd_infc_smry_id']
    ) }}
),
