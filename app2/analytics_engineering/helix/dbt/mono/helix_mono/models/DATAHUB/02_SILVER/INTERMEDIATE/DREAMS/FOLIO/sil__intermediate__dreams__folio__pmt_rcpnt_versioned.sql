{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__pmt_rcpnt_cleansed'),
            join_columns=['data_pmt_rcpnt_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__pmt_rcpnt_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_pmt_rcpnt_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pmt_rcpnt_id']
    ) }}
),
...
final as (
    select
        data_addr_rfnd_nm_ovrd_in,
        data_create_dts,
        data_create_usr_id_cd,
        data_memo_nm,
        data_pmt_id,
        data_pmt_rcpnt_addr_ln1_val,
        data_pmt_rcpnt_addr_ln2_val,
        data_pmt_rcpnt_cntry_cd,
        data_pmt_rcpnt_cty_nm,
        data_pmt_rcpnt_id,
        data_pmt_rcpnt_nm,
        data_pmt_rcpnt_pstl_cd,
        data_pmt_rcpnt_rgn_cd,
        data_pmt_rcpnt_rgn_nm,
        data_prge_dts,
        metadata_checksum,
        metadata_timestamp,
