{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__rpt_ctgy_cleansed'),
            join_columns=['data_rpt_ctgy_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__rpt_ctgy_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_rpt_ctgy_id']
    ) }}
),
...
final as (
    select
        ...
        data_rpt_ctgy_nm,
        data_rpt_ctgy_cd,
        data_rpt_ctgy_strt_dt,
        data_rpt_ctgy_end_dt,
        data_rpt_ctgy_dspl_seq_nb,
