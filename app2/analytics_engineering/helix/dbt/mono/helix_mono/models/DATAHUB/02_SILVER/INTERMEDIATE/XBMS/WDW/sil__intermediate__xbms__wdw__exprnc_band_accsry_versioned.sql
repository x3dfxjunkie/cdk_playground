{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_accsry_cleansed'),
            join_columns=['data_exprnc_band_accsry_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_accsry_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_band_accsry_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_band_accsry_id']
    ) }}
),
...
final as (
    select
        ...
        data_exprnc_band_accsry_nm,
        data_exprnc_band_accsry_avail_fr_dt,
        data_exprnc_band_accsry_avail_to_dt,
        ...
        data_updt_dts,
        data_logical_del_in,
        data_dflt_exprnc_band_accsry_in,
        metadata_checksum,
        metadata_timestamp,
