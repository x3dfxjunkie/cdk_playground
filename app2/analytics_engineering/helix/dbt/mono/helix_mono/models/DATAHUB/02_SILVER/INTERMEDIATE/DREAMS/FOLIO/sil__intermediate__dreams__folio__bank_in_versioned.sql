{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__bank_in_cleansed'),
            join_columns=['data_bank_in_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__bank_in_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_bank_in_id']
    ) }}
),
...
final as (
    select
        data_bank_in_id,
        data_bank_in_acct_dt,
        data_bank_in_dts,
        data_bank_acct_ctr_id,
        data_rgstr_nb_vl,
        data_bank_in_user_id,
        data_loc_id,
        data_chg_fund_am,
        data_chg_fund_crncy_cd,
