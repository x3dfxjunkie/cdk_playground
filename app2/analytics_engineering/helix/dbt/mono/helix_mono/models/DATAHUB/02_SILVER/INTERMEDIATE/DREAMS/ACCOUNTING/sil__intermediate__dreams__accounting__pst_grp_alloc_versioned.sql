{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__pst_grp_alloc_cleansed'),
            join_columns=['data_pst_grp_alloc_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__pst_grp_alloc_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_pst_grp_alloc_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pst_grp_alloc_id']
    ) }}
),
...
final as (
    select
        ...
        data_pst_grp_id,
        data_pst_acct_id,
        data_pst_grp_alloc_strt_dt,
        data_pst_grp_alloc_end_dt,
        data_pst_grp_alloc_ord_nb,
        data_pst_grp_alloc_pc,
        data_pst_grp_flat_alloc_am,
        data_flat_am_crncy_cd,
