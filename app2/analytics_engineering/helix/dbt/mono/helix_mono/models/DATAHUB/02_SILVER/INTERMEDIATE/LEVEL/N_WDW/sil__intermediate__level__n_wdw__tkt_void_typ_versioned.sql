{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__tkt_void_typ_cleansed'),
            join_columns=['data_tkt_void_typ_cd'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__tkt_void_typ_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_tkt_void_typ_cd
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_tkt_void_typ_cd']
    ) }}
),
