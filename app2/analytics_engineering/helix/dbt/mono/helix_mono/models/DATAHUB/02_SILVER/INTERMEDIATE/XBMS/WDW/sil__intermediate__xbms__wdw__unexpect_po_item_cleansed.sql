{{
    config(
        ...
        unique_key= ['data_unexpect_po_item_id','metadata_checksum'],
...
with source as (
    ...
    from {{ source('brz_xbms_wdw', 'unexpect_po_item') }}
...
renamed as (
    select
        src.data_unexpect_po_item_id as data_unexpect_po_item_id,
    ...
    on src.data_unexpect_po_item_id = min_source_update_datetime.data_unexpect_po_item_id
),
...
final as (
    {{ dbt_utils.deduplicate(
        relation = 'renamed',
        partition_by = 'data_unexpect_po_item_id, metadata_checksum',
