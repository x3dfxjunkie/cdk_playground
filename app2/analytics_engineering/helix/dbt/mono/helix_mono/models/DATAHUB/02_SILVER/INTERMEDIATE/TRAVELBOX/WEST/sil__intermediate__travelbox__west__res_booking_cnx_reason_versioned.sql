{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__west__res_booking_cnx_reason_cleansed'),
            join_columns=['data_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__west__res_booking_cnx_reason_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        data_id,
        data_reason,
        data_conf_doc_required,
        data_status,
        data_app_for_third_party_ins,
        data_authority_level,
        data_reason_type,
        data_repricing_date,
        data_applicable_products,
        metadata_checksum,
        metadata_timestamp,
