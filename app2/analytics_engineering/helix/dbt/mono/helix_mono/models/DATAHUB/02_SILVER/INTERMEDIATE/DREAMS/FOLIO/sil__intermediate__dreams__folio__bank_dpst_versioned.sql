{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__bank_dpst_cleansed'),
            join_columns=['data_bank_dpst_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__bank_dpst_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_bank_dpst_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_bank_dpst_id']
    ) }}
),
...
final as (
    select
        data_bank_dpst_id,
        data_bank_in_id,
        data_bank_dpst_typ_nm,
        data_bank_dpst_acct_dt,
        data_dpst_bag_id,
        data_bank_dpst_dts,
        data_cash_dpst_am,
        data_csh_dpst_crncy_cd,
        data_frgn_dpst_am,
        data_frgn_dpst_crncy_cd,
        data_wrk_off_dt,
