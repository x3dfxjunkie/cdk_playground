{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__denom_item_bnk_out_cleansed'),
            join_columns=['data_denom_item_bnk_out_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__denom_item_bnk_out_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_denom_item_bnk_out_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_denom_item_bnk_out_id']
    ) }}
),
...
final as (
    select
        ...
        data_denom_item_typ_nm,
        data_bank_out_id,
        data_denom_item_typ_cn,
        data_denom_item_am,
        data_denom_item_crncy_cd,
