{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__rev_cls_cleansed'),
            join_columns=['data_rev_cls_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__rev_cls_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_rev_cls_id']
    ) }}
),
...
final as (
    select
        ...
        data_jdo_seq_nb,
        data_rev_cls_lvl_nb,
        data_prnt_rev_cls_id,
        data_root_rev_cls_id,
        data_root_rev_cls_nm,
        metadata_checksum,
        metadata_timestamp,
