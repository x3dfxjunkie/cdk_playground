{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__non_folio_rfnd_itm_cleansed'),
            join_columns=['data_non_folio_rfnd_item_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__non_folio_rfnd_itm_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_non_folio_rfnd_item_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_non_folio_rfnd_item_id']
    ) }}
),
...
final as (
    select
        ...
        data_bank_out_id,
        data_non_folio_rfnd_item_typ_nm,
        data_non_folio_item_rfnd_cn,
        data_non_folio_item_rfnd_am,
        data_non_folio_item_crncy_cd,
