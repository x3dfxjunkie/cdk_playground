{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__resource_inventory_management__rsrc_own_ref_cleansed'),
            join_columns=['data_extnl_own_ref_val', 'data_rsrc_own_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__resource_inventory_management__rsrc_own_ref_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_extnl_own_ref_val, data_rsrc_own_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_extnl_own_ref_val', 'data_rsrc_own_id']
    ) }}
),
