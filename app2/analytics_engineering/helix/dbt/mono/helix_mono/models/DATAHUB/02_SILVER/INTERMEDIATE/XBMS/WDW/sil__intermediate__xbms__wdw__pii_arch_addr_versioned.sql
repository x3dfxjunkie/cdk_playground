{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__pii_arch_addr_cleansed'),
            join_columns=['data_addr_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__pii_arch_addr_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_addr_id']
    ) }}
),
...
final as (
    select
        ...
        data_addr_st_nm,
        data_addr_pstl_cd,
        data_exprnc_band_rcpnt_phn_nb,
        data_exprnc_band_rcpnt_nm,
        data_exprnc_band_rcpnt_phn_ext_nb,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
