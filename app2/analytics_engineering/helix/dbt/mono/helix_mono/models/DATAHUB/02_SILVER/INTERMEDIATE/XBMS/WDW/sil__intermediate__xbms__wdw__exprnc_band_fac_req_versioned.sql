{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_fac_req_cleansed'),
            join_columns=['data_exprnc_band_req_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_fac_req_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_band_req_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_band_req_id']
    ) }}
),
...
final as (
    select
        ...
        data_exprnc_band_req_ds,
        data_pkg_cmpnt_id,
        data_fac_req_expect_dlvr_dt,
        data_cost_ctr_id,
        data_wbs_elmnt_id,
        data_gnrl_ldgr_id,
        data_ovrd_exprnc_band_sts_id,
        data_avail_gi_in,
        data_exprnc_band_req_hld_in,
        data_curr_req_hld_rsn_id,
        data_merch_po_nb,
        data_exprnc_band_req_on_dmnd_in,
        data_bab_shipmt_nb,
        metadata_checksum,
        metadata_timestamp,
