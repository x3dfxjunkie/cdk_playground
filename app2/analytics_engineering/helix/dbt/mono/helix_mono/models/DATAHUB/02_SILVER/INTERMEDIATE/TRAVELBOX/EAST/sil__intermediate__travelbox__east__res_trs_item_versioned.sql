{{
    config(
        ...
        unique_key = ['data_booking_id', 'data_item_no', 'data_product_code', 'data_trs_item_no','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__east__res_trs_item_cleansed'),
            join_columns=['data_booking_id', 'data_item_no', 'data_product_code', 'data_trs_item_no'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__east__res_trs_item_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_booking_id, data_item_no, data_product_code, data_trs_item_no
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_booking_id', 'data_item_no', 'data_product_code', 'data_trs_item_no']
    ) }}
),
