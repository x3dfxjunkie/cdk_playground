{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__ship_carrier_cleansed'),
            join_columns=['data_ship_carrier_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__ship_carrier_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_ship_carrier_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_ship_carrier_id']
    ) }}
),
...
final as (
    select
        ...
        data_ship_carrier_cd,
        data_ship_carrier_nm,
        data_ship_carrier_trk_url_tx,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
