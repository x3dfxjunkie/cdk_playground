{{
    config(
        ...
        unique_key = ['data_res_mgmt_req_id', 'data_res_mgmt_rte_nm','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_reservations__res_mgmt_req_rte_cleansed'),
            join_columns=['data_res_mgmt_req_id', 'data_res_mgmt_rte_nm'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_reservations__res_mgmt_req_rte_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_res_mgmt_req_id, data_res_mgmt_rte_nm
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_res_mgmt_req_id', 'data_res_mgmt_rte_nm']
    ) }}
),
