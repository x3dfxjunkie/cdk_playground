{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_lnk_lnk_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_lnk_lnk_cleansed') }}
    {% if is_incremental() %}
