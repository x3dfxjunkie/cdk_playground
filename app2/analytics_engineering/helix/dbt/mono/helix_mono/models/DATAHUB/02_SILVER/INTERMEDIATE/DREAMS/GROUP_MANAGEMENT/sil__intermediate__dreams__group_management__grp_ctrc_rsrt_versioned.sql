{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__group_management__grp_ctrc_rsrt_cleansed'),
            join_columns=['data_grp_ctrc_rsrt_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__group_management__grp_ctrc_rsrt_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_grp_ctrc_rsrt_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_grp_ctrc_rsrt_id']
    ) }}
),
...
final as (
    select
        ...
        data_grp_cd,
        data_grp_ctrc_rsrt_addl_adlt_rt_tx,
        data_grp_ctrc_rsrt_end_dt,
        data_grp_ctrc_rsrt_id,
        data_grp_ctrc_rsrt_nm_tx,
        data_grp_ctrc_rsrt_pkg_tx,
        data_grp_ctrc_rsrt_rm_typ_tx,
        data_grp_ctrc_rsrt_rt_tx,
        data_grp_ctrc_rsrt_strt_dt,
        data_jdo_seq_nb,
        data_prge_dts,
        data_updt_dts,
        data_updt_usr_id_cd,
