{{
    config(
        ...
        unique_key = ['data_suplmnt_log_tbl_nm','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__suplmnt_log_tbl_cleansed'),
            join_columns=['data_suplmnt_log_tbl_nm'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__suplmnt_log_tbl_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_suplmnt_log_tbl_nm
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_suplmnt_log_tbl_nm']
    ) }}
),
