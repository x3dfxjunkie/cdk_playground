{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__pkg_cmpnt_cleansed'),
            join_columns=['data_pkg_cmpnt_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__pkg_cmpnt_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_pkg_cmpnt_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pkg_cmpnt_id']
    ) }}
),
...
final as (
    select
        ...
        data_pkg_cmpnt_actv_fr_dt,
        data_pkg_cmpnt_actv_until_dt,
        data_pkg_cmpnt_min_cpcty_cn,
        data_pkg_cmpnt_max_cpcty_cn,
        data_pkg_cmpnt_actv_in,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
