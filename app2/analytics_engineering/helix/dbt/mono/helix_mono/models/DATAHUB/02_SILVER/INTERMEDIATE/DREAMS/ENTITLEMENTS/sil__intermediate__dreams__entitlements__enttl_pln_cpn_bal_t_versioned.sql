{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__entitlements__enttl_pln_cpn_bal_t_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__entitlements__enttl_pln_cpn_bal_t_cleansed') }}
    {% if is_incremental() %}
