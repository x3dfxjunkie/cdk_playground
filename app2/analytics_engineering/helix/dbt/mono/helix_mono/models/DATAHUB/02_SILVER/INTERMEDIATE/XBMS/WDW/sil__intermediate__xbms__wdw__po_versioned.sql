{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__po_cleansed'),
            join_columns=['data_po_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__po_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        ...
        data_exprnc_band_mfr_id,
        data_po_sts_id,
        data_po_nb,
        data_po_placed_dt,
        data_po_expect_ship_dt,
        data_po_expect_dlvr_dt,
        data_po_actl_ship_dt,
        data_po_actl_dlvr_dt,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_logical_del_in,
        data_simba_shipmt_nb,
        data_externreceiptkey,
        metadata_checksum,
        metadata_timestamp,
