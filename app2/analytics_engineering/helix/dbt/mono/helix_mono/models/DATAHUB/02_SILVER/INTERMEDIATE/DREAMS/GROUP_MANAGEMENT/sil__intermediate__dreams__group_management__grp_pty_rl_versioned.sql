{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__group_management__grp_pty_rl_cleansed'),
            join_columns=['data_grp_cd', 'data_pty_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__group_management__grp_pty_rl_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_grp_cd, data_pty_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_grp_cd', 'data_pty_id']
    ) }}
),
...
final as (
    select
        ...
        data_pty_id,
        data_rl_nm,
        data_grp_pty_rl_dflt_trvl_agcy_in,
        data_grp_pty_rl_strt_dts,
        data_grp_pty_rl_end_dts,
        data_create_dts,
