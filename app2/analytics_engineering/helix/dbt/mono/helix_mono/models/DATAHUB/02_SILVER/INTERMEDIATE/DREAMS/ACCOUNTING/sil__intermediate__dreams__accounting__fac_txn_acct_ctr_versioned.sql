{{
    config(
        ...
        unique_key = ['data_fac_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__fac_txn_acct_ctr_cleansed'),
            join_columns=['data_fac_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__fac_txn_acct_ctr_cleansed') }}
    {% if is_incremental() %}
