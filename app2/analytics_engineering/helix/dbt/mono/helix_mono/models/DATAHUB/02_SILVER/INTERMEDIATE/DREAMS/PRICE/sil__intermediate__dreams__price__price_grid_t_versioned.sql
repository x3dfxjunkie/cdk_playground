{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__price__price_grid_t_cleansed'),
            join_columns=['data_price_grid_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__price__price_grid_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_price_grid_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_price_grid_id']
    ) }}
),
...
final as (
    select
        ...
        data_price_grid_ds,
        data_price_grid_id,
        data_price_grid_pkg_in,
        data_price_grid_typ_nm,
        data_rt_grid_cd,
        data_updt_dts,
        data_updt_usr_id_cd,
        metadata_checksum,
