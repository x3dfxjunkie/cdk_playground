{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_cleansed'),
            join_columns=['data_exprnc_band_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_band_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_band_id']
    ) }}
),
...
final as (
    select
        ...
        data_exprnc_band_uniq_id,
        data_exprnc_band_req_id,
        data_exprnc_band_ord_id,
        data_fflmt_shipmt_id,
        data_exprnc_band_pkg_id,
        data_exprnc_band_prt_nm,
        data_exprnc_band_gst_cnfirm_in,
        data_exprnc_band_seq_nb,
        data_exprnc_band_cstm_prt_nm_in,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_logical_del_in,
        data_curr_exprnc_band_lnk_id,
        data_exprnc_band_rtl_in,
        data_exprnc_band_ovrd_prt_nm,
        data_orig_exprnc_band_lnk_id,
        data_opt_out_in,
        data_exprnc_band_opt_out_rsn_id,
        data_exprnc_band_gst_sku_ovrd_in,
        data_incog_req_id,
        metadata_checksum,
        metadata_timestamp,
