{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__lock_bx_pmt_cleansed'),
            join_columns=['data_pmt_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__lock_bx_pmt_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        data_pmt_id,
        data_lckbx_chk_nb_vl,
        data_lckbx_btch_extnl_nb_val,
        data_lckbx_extnl_btch_dts,
