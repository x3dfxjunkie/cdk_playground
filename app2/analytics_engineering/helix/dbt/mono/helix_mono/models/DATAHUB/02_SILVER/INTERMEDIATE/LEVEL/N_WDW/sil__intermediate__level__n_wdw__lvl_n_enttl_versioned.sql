{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__level__n_wdw__lvl_n_enttl_cleansed'),
            join_columns=['data_lvl_n_enttl_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__level__n_wdw__lvl_n_enttl_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_lvl_n_enttl_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_lvl_n_enttl_id']
    ) }}
),
...
final as (
    select
        ...
        data_lvl_n_enttl_purch_dt,
        data_lvl_n_exp_dt,
        data_lvl_n_enttl_sls_price,
        data_lvl_n_enttl_sls_tax,
        data_lvl_n_iss_rsn_tx,
        data_rsrt_res_strt_dt,
        data_rsrt_res_end_dt,
        data_rsrt_res_exp_dt,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_lgcl_del_in,
        data_lvl_n_enttl_typ,
        data_lvl_n_enttl_earnd_price,
        metadata_checksum,
        metadata_timestamp,
