{{
    config(
        ...
        unique_key = ['data_blk_cd','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__group_management__grp_blk_cleansed'),
            join_columns=['data_blk_cd'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__group_management__grp_blk_cleansed') }}
    {% if is_incremental() %}
