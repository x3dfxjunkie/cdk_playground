{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__ffld_exprnc_band_cleansed'),
            join_columns=['data_ffld_exprnc_band_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__ffld_exprnc_band_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_ffld_exprnc_band_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_ffld_exprnc_band_id']
    ) }}
),
...
final as (
    select
        ...
        data_shipmt_case_id,
        data_exprnc_band_public_id,
        data_exprnc_band_extnl_nb,
        data_exprnc_band_secure_id,
        data_exprnc_band_short_rnge_tag_nb,
        data_exprnc_band_long_rnge_tag_nb,
        data_exprnc_band_lot_nb,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_logical_del_in,
        data_curr_exprnc_band_sts_id,
        data_curr_dsny_fac_id,
        data_fflmt_shipmt_case_id,
        data_rtrn_dsny_fac_id,
        data_exprnc_band_rtrn_dt,
        data_cost_ctr_id,
        data_wbs_elmnt_id,
        data_gnrl_ldgr_id,
        data_fflmt_procurmt_rsn_id,
        data_exprnc_band_dsny_prod_sfx_id,
        data_curr_exprnc_band_sub_sts_id,
        data_fflmt_shipmt_id,
        data_build_band_in,
        data_public_key,
        metadata_checksum,
        metadata_timestamp,
