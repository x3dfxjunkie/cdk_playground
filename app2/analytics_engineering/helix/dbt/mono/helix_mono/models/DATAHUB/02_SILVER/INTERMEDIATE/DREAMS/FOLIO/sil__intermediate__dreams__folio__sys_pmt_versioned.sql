{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__sys_pmt_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__sys_pmt_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        data_pmt_id,
        data_sys_pmt_ext_txn_nb_vl,
        data_sys_pmt_ext_res_nb_val,
        data_sys_pmt_btch_dts,
