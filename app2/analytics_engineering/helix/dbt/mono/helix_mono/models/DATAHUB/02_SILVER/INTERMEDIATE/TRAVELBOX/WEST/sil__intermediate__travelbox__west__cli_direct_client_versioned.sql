{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__west__cli_direct_client_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__west__cli_direct_client_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        data_exclude_archiving,
        data_client_id,
        data_profile_id,
        data_grade,
        data_do_not_contact,
        data_alloc_user_id,
        data_exclude_from_mailings,
        data_vip,
        data_sales_allowed,
