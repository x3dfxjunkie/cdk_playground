{{
    config(
        ...
        unique_key = ['data_exprnc_band_pkg_cmpnt_id','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_pkg_cmpnt_cleansed'),
            join_columns=['data_exprnc_band_pkg_cmpnt_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_pkg_cmpnt_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_band_pkg_cmpnt_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_band_pkg_cmpnt_id']
    ) }}
),
