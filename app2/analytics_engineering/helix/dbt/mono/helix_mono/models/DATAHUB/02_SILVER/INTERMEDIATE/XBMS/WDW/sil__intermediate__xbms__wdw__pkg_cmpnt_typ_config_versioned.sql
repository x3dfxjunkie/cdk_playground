{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__pkg_cmpnt_typ_config_cleansed'),
            join_columns=['data_pkg_cmpnt_typ_config_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__pkg_cmpnt_typ_config_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_pkg_cmpnt_typ_config_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pkg_cmpnt_typ_config_id']
    ) }}
),
...
final as (
    select
        ...
        data_prnt_pkg_cmpnt_typ_id,
        data_chld_pkg_cmpnt_typ_id,
        data_pkg_cmpnt_for_ea_prnt_cmpnt_cn,
        data_avail_to_gst_in,
        data_pkg_cmpnt_typ_config_actv_in,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
