{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__entitlements__pos_cpn_meal_prd_t_cleansed'),
            join_columns=['data_pos_cpn_meal_prd_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__entitlements__pos_cpn_meal_prd_t_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_pos_cpn_meal_prd_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_pos_cpn_meal_prd_id']
    ) }}
),
...
final as (
    select
        data_cpn_meal_prd_nm,
        data_create_dts,
        data_create_usr_id_cd,
        data_pos_cpn_meal_prd_end_dts,
        data_pos_cpn_meal_prd_id,
        data_pos_cpn_meal_prd_strt_dts,
        data_pos_cpn_prfl_id,
        data_pos_meal_prd_cpn_cn,
        metadata_checksum,
        metadata_timestamp,
