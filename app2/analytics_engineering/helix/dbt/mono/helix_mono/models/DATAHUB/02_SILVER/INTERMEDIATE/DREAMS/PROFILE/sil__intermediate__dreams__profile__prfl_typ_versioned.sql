{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__profile__prfl_typ_cleansed'),
            join_columns=['data_prfl_typ_nm'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__profile__prfl_typ_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_prfl_typ_nm']
    ) }}
),
