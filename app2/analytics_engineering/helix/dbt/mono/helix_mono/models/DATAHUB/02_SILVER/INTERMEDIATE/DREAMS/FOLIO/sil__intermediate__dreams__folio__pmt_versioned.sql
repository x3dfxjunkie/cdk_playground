{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__pmt_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__pmt_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        ...
        data_folio_txn_typ_cd,
        data_ovrd_rfnd_pmt_meth_in,
        data_pmt_pst_st_nm,
        data_wrk_loc_id,
