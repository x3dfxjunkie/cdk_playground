{{
    config(
        ...
        unique_key = ['data_tax_exmpt_typ_nm','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__tax_exmpt_typ_cleansed'),
            join_columns=['data_tax_exmpt_typ_nm'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__tax_exmpt_typ_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_tax_exmpt_typ_nm
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_tax_exmpt_typ_nm']
    ) }}
),
