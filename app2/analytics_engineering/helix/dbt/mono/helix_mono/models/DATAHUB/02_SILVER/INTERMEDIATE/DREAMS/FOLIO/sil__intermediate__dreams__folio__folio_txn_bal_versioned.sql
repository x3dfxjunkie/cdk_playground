{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__folio_txn_bal_cleansed'),
            join_columns=['data_folio_id', 'data_folio_txn_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__folio_txn_bal_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_folio_id, data_folio_txn_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_folio_id', 'data_folio_txn_id']
    ) }}
),
