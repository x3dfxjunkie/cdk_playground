...
with source as (
    ...
    from {{ source('brz_dreams_accounting', 'sap_acct') }}
...
renamed as (
    select
        TRIM(src.data_sap_gl_acct_nm) as data_sap_gl_acct_nm,
    ...
    on src.data_sap_gl_acct_nb = min_source_update_datetime.data_sap_gl_acct_nb
),
...
final as (
    {{ dbt_utils.deduplicate(
        relation = 'renamed',
        partition_by = 'data_sap_gl_acct_nb, metadata_checksum',
