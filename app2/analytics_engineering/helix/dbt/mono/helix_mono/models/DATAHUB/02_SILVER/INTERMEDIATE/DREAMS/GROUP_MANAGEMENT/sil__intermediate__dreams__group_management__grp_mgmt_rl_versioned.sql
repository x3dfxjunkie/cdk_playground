{{
    config(
        ...
        unique_key = ['data_rl_nm','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__group_management__grp_mgmt_rl_cleansed'),
            join_columns=['data_rl_nm'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__group_management__grp_mgmt_rl_cleansed') }}
    {% if is_incremental() %}
