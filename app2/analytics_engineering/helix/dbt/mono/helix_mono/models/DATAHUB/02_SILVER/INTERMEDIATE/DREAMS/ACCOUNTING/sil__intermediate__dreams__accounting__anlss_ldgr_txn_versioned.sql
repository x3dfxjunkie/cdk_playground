{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__anlss_ldgr_txn_cleansed'),
            join_columns=['data_anlss_ldgr_txn_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__anlss_ldgr_txn_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_anlss_ldgr_txn_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_anlss_ldgr_txn_id']
    ) }}
),
...
final as (
    select
        ...
        data_folio_txn_typ_nm,
        data_acct_dt,
        data_acct_txn_pst_dts,
        data_txn_prmy_pty_nm,
        data_bus_org_id,
        data_wrk_loc_id,
        data_folio_id,
        data_folio_item_id,
        data_src_acct_ctr_id,
        data_src_acct_ctr_sap_id,
        data_sap_dstr_chan_nm,
        data_tax_exmpt_txn_in,
