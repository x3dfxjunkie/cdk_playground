{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__acct_ctr_sap_cleansed'),
            join_columns=['data_acct_ctr_sap_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__acct_ctr_sap_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_acct_ctr_sap_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_acct_ctr_sap_id']
    ) }}
),
...
final as (
    select
        data_acct_ctr_id,
        data_acct_ctr_sap_end_dt,
        data_acct_ctr_sap_id,
        data_acct_ctr_sap_strt_dt,
        data_create_dts,
