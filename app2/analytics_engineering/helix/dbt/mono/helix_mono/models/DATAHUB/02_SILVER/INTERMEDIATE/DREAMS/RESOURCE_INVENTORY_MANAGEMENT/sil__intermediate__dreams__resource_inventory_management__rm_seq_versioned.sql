{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__resource_inventory_management__rm_seq_cleansed'),
            join_columns=['data_rsrc_id', 'data_rsrt_seq_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__resource_inventory_management__rm_seq_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_rsrc_id, data_rsrt_seq_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_rsrc_id', 'data_rsrt_seq_id']
    ) }}
),
