...
with source as (
    select * from {{ source('brz_xbms_wdw', 'suplmnt_log_tbl') }}
...
renamed as (
    select
        TRIM(src.data_suplmnt_log_tbl_nm) as data_suplmnt_log_tbl_nm,
    ...
    on src.data_suplmnt_log_tbl_nm = min_source_update_datetime.data_suplmnt_log_tbl_nm
),
...
final as (
    {{ dbt_utils.deduplicate(
        relation = 'renamed',
        partition_by = 'data_suplmnt_log_tbl_nm, metadata_checksum',
