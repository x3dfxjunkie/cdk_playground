{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__guest__txn_org_pty_cleansed'),
            join_columns=['data_txn_org_pty_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__guest__txn_org_pty_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_txn_org_pty_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_txn_org_pty_id']
    ) }}
),
