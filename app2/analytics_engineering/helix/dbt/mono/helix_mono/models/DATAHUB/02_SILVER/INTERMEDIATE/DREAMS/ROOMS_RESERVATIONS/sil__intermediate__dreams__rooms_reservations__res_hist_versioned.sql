{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_reservations__res_hist_cleansed'),
            join_columns=['data_res_hist_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_reservations__res_hist_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_res_hist_id']
    ) }}
),
...
final as (
    select
        ...
        data_proc_dts,
        data_res_hist_id,
        data_res_hist_loc_nm,
        data_res_hist_proc_ds,
        data_res_hist_tx,
        data_tc_grp_nm,
        data_tc_id,
        data_tps_id,
