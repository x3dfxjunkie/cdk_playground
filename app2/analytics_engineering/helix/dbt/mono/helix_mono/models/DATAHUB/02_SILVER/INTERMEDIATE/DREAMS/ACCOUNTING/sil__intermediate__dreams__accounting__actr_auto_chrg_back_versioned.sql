{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__accounting__actr_auto_chrg_back_cleansed'),
            join_columns=['data_actr_auto_chrg_back_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__accounting__actr_auto_chrg_back_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_actr_auto_chrg_back_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_actr_auto_chrg_back_id']
    ) }}
),
