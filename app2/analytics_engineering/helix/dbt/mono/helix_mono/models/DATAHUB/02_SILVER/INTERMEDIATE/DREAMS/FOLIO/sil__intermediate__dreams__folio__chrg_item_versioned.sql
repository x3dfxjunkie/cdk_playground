{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__folio__chrg_item_cleansed'),
            join_columns=['data_chrg_item_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__folio__chrg_item_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_chrg_item_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_chrg_item_id']
    ) }}
),
...
final as (
    select
        data_chrg_item_id,
        data_chrg_id,
        data_chrg_item_typ_nm,
        data_rev_typ_id,
        data_rev_typ_nm,
        data_splt_chrg_item_in,
        data_tax_item_in,
        data_chrg_item_am,
        data_chrg_item_crncy_cd,
        data_create_usr_id_cd,
        data_create_dts,
        data_updt_usr_id_cd,
        data_updt_dts,
        data_jdo_seq_nb,
        data_chrg_item_inactv_dts,
        data_rflt_dts,
        data_prge_dts,
        data_chrg_item_ovrd_dts,
        metadata_checksum,
        metadata_timestamp,
