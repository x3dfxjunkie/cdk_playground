{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__stg_ffld_exprnc_band_cleansed'),
            join_columns=['data_stg_ffld_exprnc_band_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__stg_ffld_exprnc_band_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_stg_ffld_exprnc_band_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_stg_ffld_exprnc_band_id']
    ) }}
),
...
final as (
    select
        ...
        data_stg_exprnc_band_public_id,
        data_stg_exprnc_band_secure_id,
        data_stg_exprnc_band_batch_id,
        data_stg_exprnc_band_prchs_ord,
        data_create_dts,
        data_updt_dts,
        data_create_usr_id,
        data_updt_usr_id,
