{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__family_grp_cleansed'),
            join_columns=['data_fam_cd'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__family_grp_cleansed') }}
    {% if is_incremental() %}
