{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__guest_link__xband_id_cleansed'),
            join_columns=['data_exprnc_media_id', 'data_xband_id_typ_nm', 'data_xband_id_vl'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__guest_link__xband_id_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_media_id, data_xband_id_typ_nm, data_xband_id_vl
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_media_id', 'data_xband_id_typ_nm', 'data_xband_id_vl']
    ) }}
),
