{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__dsny_fac_cleansed'),
            join_columns=['data_dsny_fac_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__dsny_fac_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_dsny_fac_id']
    ) }}
),
...
final as (
    select
        ...
        data_fac_id,
        data_dsny_fac_nm,
        data_dsny_fac_avail_for_bulk_ord_in,
        data_dsny_fac_for_rcv_gst_ord_in,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
