{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__resource_inventory_management__rm_cleansed'),
            join_columns=['data_rsrc_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__resource_inventory_management__rm_cleansed') }}
    {% if is_incremental() %}
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_rsrc_id']
    ) }}
),
...
final as (
    select
        ...
        data_jdo_seq_nb,
        data_linen_chng_intrvl_nb,
        data_lkoff_rsrc_id,
        data_max_adlt_ocpncy_cn,
        data_rm_ds,
        data_rm_end_dt,
        data_rm_id_vl,
        data_rm_nm,
