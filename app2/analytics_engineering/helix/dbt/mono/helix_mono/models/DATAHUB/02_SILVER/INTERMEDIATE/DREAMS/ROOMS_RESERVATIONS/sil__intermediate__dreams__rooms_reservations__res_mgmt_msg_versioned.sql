{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__dreams__rooms_reservations__res_mgmt_msg_cleansed'),
...
with cleansed as (
    select * from {{ ref('sil__intermediate__dreams__rooms_reservations__res_mgmt_msg_cleansed') }}
    {% if is_incremental() %}
...
final as (
    select
        ...
        data_res_mgmt_dlvr_sts_nm,
        data_rcpnt_txn_idvl_pty_id,
        data_transact_seq,
        metadata_checksum,
        metadata_timestamp,
