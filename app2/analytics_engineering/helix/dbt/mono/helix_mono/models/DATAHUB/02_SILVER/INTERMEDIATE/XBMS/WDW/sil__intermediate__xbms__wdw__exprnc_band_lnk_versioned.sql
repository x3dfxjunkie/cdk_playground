{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__exprnc_band_lnk_cleansed'),
            join_columns=['data_exprnc_band_lnk_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__exprnc_band_lnk_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_exprnc_band_lnk_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_exprnc_band_lnk_id']
    ) }}
),
...
final as (
    select
        ...
        data_exprnc_band_lnk_uniq_id,
        data_gst_src_sys_native_id,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
        data_logical_del_in,
        data_src_sys_tx,
        data_gst_prfl_frst_nm,
        data_gst_prfl_lst_nm,
        data_incog_req_id,
        metadata_checksum,
        metadata_timestamp,
