{{
    config(
        ...
        unique_key = ['data_attribute_id', 'data_booking_id', 'data_index_no','metadata_checksum'],
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__west__res_bkg_usr_defined_type_cleansed'),
            join_columns=['data_attribute_id', 'data_booking_id', 'data_index_no'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__west__res_bkg_usr_defined_type_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_attribute_id, data_booking_id, data_index_no
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_attribute_id', 'data_booking_id', 'data_index_no']
    ) }}
),
