{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__travelbox__west__gen_usr_defined_type_cleansed'),
            join_columns=['data_contract_id', 'data_user_defined_type_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__travelbox__west__gen_usr_defined_type_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_contract_id, data_user_defined_type_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_contract_id', 'data_user_defined_type_id']
    ) }}
),
