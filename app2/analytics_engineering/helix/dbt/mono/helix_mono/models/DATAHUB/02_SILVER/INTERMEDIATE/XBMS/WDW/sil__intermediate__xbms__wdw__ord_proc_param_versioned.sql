{{
    config(
        ...
        pre_hook = "{{ macro_update_target_version_endtime(
            cleansed_table = ref('sil__intermediate__xbms__wdw__ord_proc_param_cleansed'),
            join_columns=['data_ord_proc_param_id'],
...
with cleansed as (
    select * from {{ ref('sil__intermediate__xbms__wdw__ord_proc_param_cleansed') }}
    {% if is_incremental() %}
...
renamed as (
    select
        *,
        row_number() over (
            partition by data_ord_proc_param_id
                order by
...
versioned as (
    {{ macro_create_versions(
        ...
        partition_columns = ['data_ord_proc_param_id']
    ) }}
),
...
final as (
    select
        ...
        data_ord_proc_param_vl,
        data_ord_proc_param_strt_dts,
        data_ord_proc_param_end_dts,
        data_create_usr_id,
        data_create_dts,
        data_updt_usr_id,
        data_updt_dts,
